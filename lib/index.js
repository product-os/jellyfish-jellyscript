...
// $$formula: 'AGGREGATE(this.links[\'<LINK_VERB>\'], PARTIAL(FLIP(PROPERTY), "<SOURCE_PATH>"))'
...
exports.getTypeTriggers = (typeCard) => {
	...
	for (const path of card.getFormulasPaths(typeCard.data.schema)) {
		...
		if (_.isMatch(ast, LINKS_AGGREGATE_BASE_AST)) {
			const arg = runAST(ast.arguments[1].arguments[1], {
				context: {},
				input: {}
			})

			const linkVerb = ast.arguments[0].property.value
