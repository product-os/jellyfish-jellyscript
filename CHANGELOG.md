# Change Log

All notable changes to this project will be documented in this file
automatically by Versionist. DO NOT EDIT THIS FILE MANUALLY!
This project adheres to [Semantic Versioning](http://semver.org/).

## 7.11.5 - 2023-08-28

* Update external types [Josh Bowling]

## 7.11.4 - 2023-08-28

* Add missing @types/lodash dependency [Josh Bowling]

## 7.11.3 - 2023-08-28

* Update typescript to 5.2.2 [Josh Bowling]

## 7.11.2 - 2023-08-28

* Remove typedoc [Josh Bowling]

## 7.11.1 - 2023-04-02

* Bump deps [Josh Bowling]

## 7.11.0 - 2023-02-02

* Update Node.js to 18.14.0 [Renovate Bot]

## 7.10.0 - 2023-01-27

* Update github/codeql-action digest to 3ebbd71 [Renovate Bot]

## 7.9.0 - 2023-01-26

* Update github/codeql-action digest to 436dbd9 [Renovate Bot]

## 7.8.1 - 2023-01-18

* Update dependency rimraf to 4.1.1 [Renovate Bot]

## 7.8.0 - 2023-01-18

* Update github/codeql-action digest to a34ca99 [Renovate Bot]

## 7.7.0 - 2023-01-06

* Update Node.js to 18.13.0 [Renovate Bot]

## 7.6.0 - 2023-01-05

* Update actions/checkout digest to ac59398 [Renovate Bot]

## 7.5.1 - 2022-12-22

* Update dependency @types/object-hash to 3.0.2 [Renovate Bot]

## 7.5.0 - 2022-12-22

* Update Node.js to 18.12.1 [Renovate Bot]

## 7.4.0 - 2022-12-21

* Update Node.js to 16.19.0 [Renovate Bot]

## 7.3.0 - 2022-12-21

* Pin dependencies [Renovate Bot]

## 7.2.1 - 2022-12-14

* Update @balena/lint from 6.2.0 to 6.2.1 [Josh Bowling]

## 7.2.0 - 2022-12-13

* Update Node.js to 16.18.1 [Renovate Bot]

## 7.1.144 - 2022-12-13

* CI: Stop extending renovate config [Josh Bowling]

## 7.1.143 - 2022-11-05

* Update @types/jest from 29.2.0 to 29.2.2 [renovate[bot]]

## 7.1.142 - 2022-11-03

* Update typedoc from 0.23.19 to 0.23.20 [renovate[bot]]

## 7.1.141 - 2022-10-29

* Update jest from 29.2.1 to 29.2.2 [renovate[bot]]

## 7.1.140 - 2022-10-28

* Update typedoc from 0.23.18 to 0.23.19 [renovate[bot]]

## 7.1.139 - 2022-10-25

* Update autumndb from 22.2.165 to 22.2.167 [renovate[bot]]

## 7.1.138 - 2022-10-24

* Update autumndb from 22.2.164 to 22.2.165 [renovate[bot]]

## 7.1.137 - 2022-10-24

* Update autumndb from 22.2.162 to 22.2.164 [renovate[bot]]

## 7.1.136 - 2022-10-24

* Update autumndb from 22.2.160 to 22.2.162 [renovate[bot]]

## 7.1.135 - 2022-10-24

* Docs: Remove badge from readme [Josh Bowling]

## 7.1.134 - 2022-10-24

* Update autumndb from 22.2.158 to 22.2.160 [renovate[bot]]

## 7.1.133 - 2022-10-23

* Update typedoc from 0.23.17 to 0.23.18 [renovate[bot]]

## 7.1.132 - 2022-10-22

* Update autumndb from 22.2.154 to 22.2.158 [renovate[bot]]

## 7.1.131 - 2022-10-22

* Update jest monorepo [renovate[bot]]

## 7.1.130 - 2022-10-22

* Update simple-git-hooks from 2.8.0 to 2.8.1 [renovate[bot]]

## 7.1.129 - 2022-10-18

* Update autumndb from 22.2.153 to 22.2.154 [renovate[bot]]

## 7.1.128 - 2022-10-18

* Update autumndb from 22.2.151 to 22.2.153 [renovate[bot]]

## 7.1.127 - 2022-10-18

* Update autumndb from 22.2.146 to 22.2.151 [renovate[bot]]

## 7.1.126 - 2022-10-18

* Update typedoc from 0.23.16 to 0.23.17 [renovate[bot]]

## 7.1.125 - 2022-10-15

* Update jest from 29.1.2 to 29.2.0 [renovate[bot]]

## 7.1.124 - 2022-10-11

* Update autumndb from 22.2.145 to 22.2.146 [renovate[bot]]

## 7.1.123 - 2022-10-11

* Update autumndb from 22.2.141 to 22.2.145 [renovate[bot]]

## 7.1.122 - 2022-10-11

* Update autumndb from 22.2.140 to 22.2.141 [renovate[bot]]

## 7.1.121 - 2022-10-10

* Update autumndb from 22.2.139 to 22.2.140 [renovate[bot]]

## 7.1.120 - 2022-10-10

* Update typedoc from 0.23.15 to 0.23.16 [renovate[bot]]

## 7.1.119 - 2022-10-10

* Update autumndb from 22.2.138 to 22.2.139 [renovate[bot]]

## 7.1.118 - 2022-10-10

* Update autumndb from 22.2.135 to 22.2.138 [renovate[bot]]

## 7.1.117 - 2022-10-10

* CI: Move tests to test/unit [Josh Bowling]

## 7.1.116 - 2022-10-09

* Update autumndb from 22.2.134 to 22.2.135 [renovate[bot]]

## 7.1.115 - 2022-10-09

* Update autumndb from 22.2.133 to 22.2.134 [renovate[bot]]

## 7.1.114 - 2022-10-09

* Update autumndb from 22.2.132 to 22.2.133 [renovate[bot]]

## 7.1.113 - 2022-10-09

* Update autumndb from 22.2.130 to 22.2.132 [renovate[bot]]

## 7.1.112 - 2022-10-09

* Update autumndb from 22.2.129 to 22.2.130 [renovate[bot]]

## 7.1.111 - 2022-10-09

* Update autumndb from 22.2.124 to 22.2.129 [renovate[bot]]

## 7.1.110 - 2022-10-08

* Dev: Remove tsx from lint-staged [Josh Bowling]

## 7.1.109 - 2022-10-08

* Dev: Use prettier for json/yml [Josh Bowling]

## 7.1.108 - 2022-10-08

* Update @types/jest from 29.1.1 to 29.1.2 [renovate[bot]]

## 7.1.107 - 2022-10-07

* Update autumndb from 22.2.123 to 22.2.124 [renovate[bot]]

## 7.1.106 - 2022-10-06

* Update autumndb from 22.2.122 to 22.2.123 [renovate[bot]]

## 7.1.105 - 2022-10-04

* Update autumndb from 22.2.121 to 22.2.122 [renovate[bot]]

## 7.1.104 - 2022-10-04

* Update autumndb from 22.2.119 to 22.2.121 [renovate[bot]]

## 7.1.103 - 2022-10-03

* Update autumndb from 22.2.115 to 22.2.119 [renovate[bot]]

## 7.1.102 - 2022-10-03

* Update autumndb from 22.2.114 to 22.2.115 [renovate[bot]]

## 7.1.101 - 2022-10-02

* Update autumndb from 22.2.112 to 22.2.114 [renovate[bot]]

## 7.1.100 - 2022-10-01

* Update autumndb from 22.2.111 to 22.2.112 [renovate[bot]]

## 7.1.99 - 2022-10-01

* Update autumndb from 22.2.108 to 22.2.111 [renovate[bot]]

## 7.1.98 - 2022-10-01

* Update autumndb from 22.2.105 to 22.2.108 [renovate[bot]]

## 7.1.97 - 2022-10-01

* Update autumndb from 22.2.103 to 22.2.105 [renovate[bot]]

## 7.1.96 - 2022-10-01

* Update autumndb from 22.2.101 to 22.2.103 [renovate[bot]]

## 7.1.95 - 2022-10-01

* Update jest monorepo [renovate[bot]]

## 7.1.94 - 2022-10-01

* Update autumndb from 22.2.99 to 22.2.101 [renovate[bot]]

## 7.1.93 - 2022-10-01

* Update typescript from 4.8.3 to 4.8.4 [renovate[bot]]

## 7.1.92 - 2022-10-01

* Update ts-jest from 29.0.2 to 29.0.3 [renovate[bot]]

## 7.1.91 - 2022-09-28

* Update autumndb from 22.2.98 to 22.2.99 [renovate[bot]]

## 7.1.90 - 2022-09-28

* Update autumndb from 22.2.95 to 22.2.98 [renovate[bot]]

## 7.1.89 - 2022-09-28

* Change engines to >=16 to enable node 18 tests/usage [Page-]

## 7.1.88 - 2022-09-28

* Update autumndb from 22.2.93 to 22.2.95 [renovate[bot]]

## 7.1.87 - 2022-09-27

* Update autumndb from 22.2.92 to 22.2.93 [renovate[bot]]

## 7.1.86 - 2022-09-27

* Update autumndb from 22.2.87 to 22.2.92 [renovate[bot]]

## 7.1.85 - 2022-09-27

* Update autumndb from 22.2.85 to 22.2.87 [renovate[bot]]

## 7.1.84 - 2022-09-27

* Rely on flowzone for docs publishing [Page-]

## 7.1.83 - 2022-09-24

* Update autumndb from 22.2.84 to 22.2.85 [renovate[bot]]

## 7.1.82 - 2022-09-24

* Update autumndb from 22.2.80 to 22.2.84 [renovate[bot]]

## 7.1.81 - 2022-09-24

* Update autumndb from 22.2.78 to 22.2.80 [renovate[bot]]

## 7.1.80 - 2022-09-24

* Update ts-jest from 29.0.1 to 29.0.2 [renovate[bot]]

## 7.1.79 - 2022-09-24

* Update autumndb from 22.2.76 to 22.2.78 [renovate[bot]]

## 7.1.78 - 2022-09-23

* Update autumndb from 22.2.75 to 22.2.76 [renovate[bot]]

## 7.1.77 - 2022-09-23

* Update autumndb from 22.2.73 to 22.2.75 [renovate[bot]]

## 7.1.76 - 2022-09-20

* Update autumndb from 22.2.72 to 22.2.73 [renovate[bot]]

## 7.1.75 - 2022-09-19

* Update autumndb from 22.2.71 to 22.2.72 [renovate[bot]]

## 7.1.74 - 2022-09-19

* Update autumndb from 22.2.65 to 22.2.71 [renovate[bot]]

## 7.1.73 - 2022-09-19

* Update autumndb from 22.2.62 to 22.2.65 [renovate[bot]]

## 7.1.72 - 2022-09-19

* Move CodeQL workflow config [Josh Bowling]

## 7.1.71 - 2022-09-19

* Update autumndb from 22.2.59 to 22.2.62 [renovate[bot]]

## 7.1.70 - 2022-09-19

* Add CodeQL workflow [Josh Bowling]

## 7.1.69 - 2022-09-19

* Update autumndb from 22.2.57 to 22.2.59 [renovate[bot]]

## 7.1.68 - 2022-09-19

* Dev: Add .nvmrc [Josh Bowling]

## 7.1.67 - 2022-09-19

* Update autumndb from 22.2.55 to 22.2.57 [renovate[bot]]

## 7.1.66 - 2022-09-18

* Update typedoc from 0.23.14 to 0.23.15 [renovate[bot]]

## 7.1.65 - 2022-09-18

* Update autumndb from 22.2.53 to 22.2.55 [renovate[bot]]

## 7.1.64 - 2022-09-18

* Update autumndb from 22.2.52 to 22.2.53 [renovate[bot]]

## 7.1.63 - 2022-09-17

* Update autumndb from 22.2.49 to 22.2.52 [renovate[bot]]

## 7.1.62 - 2022-09-17

* Update autumndb from 22.2.47 to 22.2.49 [renovate[bot]]

## 7.1.61 - 2022-09-17

* Update autumndb from 22.2.45 to 22.2.47 [renovate[bot]]

## 7.1.60 - 2022-09-17

* Update ts-jest from 29.0.0 to 29.0.1 [renovate[bot]]

## 7.1.59 - 2022-09-17

* Update @types/jest from 29.0.1 to 29.0.3 [renovate[bot]]

## 7.1.58 - 2022-09-16

* Update autumndb from 22.2.44 to 22.2.45 [renovate[bot]]

## 7.1.57 - 2022-09-13

* Update autumndb from 22.2.42 to 22.2.44 [renovate[bot]]

## 7.1.56 - 2022-09-13

* Update autumndb from 22.2.39 to 22.2.42 [renovate[bot]]

## 7.1.55 - 2022-09-13

* Update autumndb from 22.2.38 to 22.2.39 [renovate[bot]]

## 7.1.54 - 2022-09-13

* Update node engine version [Josh Bowling]

## 7.1.53 - 2022-09-13

* CI: Move to Flowzone [Josh Bowling]

## 7.1.52 - 2022-09-12

* Update autumndb from 22.2.37 to 22.2.38 [renovate[bot]]

## 7.1.51 - 2022-09-12

* Update autumndb from 22.2.36 to 22.2.37 [renovate[bot]]

## 7.1.50 - 2022-09-12

* Update autumndb from 22.2.34 to 22.2.36 [renovate[bot]]

## 7.1.49 - 2022-09-12

* Update autumndb from 22.2.26 to 22.2.34 [renovate[bot]]

## 7.1.48 - 2022-09-12

* Update @types/jest from 29.0.0 to 29.0.1 [renovate[bot]]

## 7.1.47 - 2022-09-11

* Update autumndb from 22.2.25 to 22.2.26 [renovate[bot]]

## 7.1.46 - 2022-09-11

* Update autumndb from 22.2.23 to 22.2.25 [renovate[bot]]

## 7.1.45 - 2022-09-11

* Update autumndb from 22.2.22 to 22.2.23 [renovate[bot]]

## 7.1.44 - 2022-09-10

* Update autumndb from 22.2.19 to 22.2.22 [renovate[bot]]

## 7.1.43 - 2022-09-10

* Update autumndb from 22.2.18 to 22.2.19 [renovate[bot]]

## 7.1.42 - 2022-09-10

* Update jest from 29.0.2 to 29.0.3 [renovate[bot]]

## 7.1.41 - 2022-09-10

* Update autumndb from 22.2.10 to 22.2.18 [renovate[bot]]

## 7.1.40 - 2022-09-10

* Update autumndb from 22.2.7 to 22.2.10 [renovate[bot]]

## 7.1.39 - 2022-09-10

* Update jest to v29 [Josh Bowling]

## 7.1.38 - 2022-09-10

* Update autumndb from 22.2.6 to 22.2.7 [renovate[bot]]

## 7.1.37 - 2022-09-10

* Update autumndb from 22.1.28 to 22.2.6 [renovate[bot]]

## 7.1.36 - 2022-09-10

* Update typescript to 4.8.3 [Josh Bowling]

## 7.1.35 - 2022-09-10

* Update typedoc from 0.23.10 to 0.23.14 [renovate[bot]]

## 7.1.34 - 2022-09-10

* Update @types/jest from 28.1.7 to 28.1.8 [renovate[bot]]

## 7.1.33 - 2022-08-23

* Update autumndb from 22.1.27 to 22.1.28 [renovate[bot]]

## 7.1.32 - 2022-08-22

* Update autumndb from 22.1.26 to 22.1.27 [renovate[bot]]

## 7.1.31 - 2022-08-22

* Update autumndb from 22.1.25 to 22.1.26 [renovate[bot]]

## 7.1.30 - 2022-08-21

* Update autumndb from 22.1.14 to 22.1.25 [renovate[bot]]

## 7.1.29 - 2022-08-21

* Update ts-jest from 28.0.7 to 28.0.8 [renovate[bot]]

## 7.1.28 - 2022-08-20

* Update @types/jest from 28.1.6 to 28.1.7 [renovate[bot]]

## 7.1.27 - 2022-08-18

* Update autumndb from 22.1.13 to 22.1.14 [renovate[bot]]

## 7.1.26 - 2022-08-17

* Update autumndb from 22.1.11 to 22.1.13 [renovate[bot]]

## 7.1.25 - 2022-08-16

* Update autumndb from 22.1.8 to 22.1.11 [renovate[bot]]

## 7.1.24 - 2022-08-13

* Update autumndb from 22.1.6 to 22.1.8 [renovate[bot]]

## 7.1.23 - 2022-08-13

* Update autumndb from 22.1.2 to 22.1.6 [renovate[bot]]

## 7.1.22 - 2022-08-13

* Update autumndb from 22.1.1 to 22.1.2 [renovate[bot]]

## 7.1.21 - 2022-08-13

* Update autumndb from 22.1.0 to 22.1.1 [renovate[bot]]

## 7.1.20 - 2022-08-10

* Update autumndb from 22.0.8 to 22.1.0 [renovate[bot]]

## 7.1.19 - 2022-08-08

* Update autumndb from 22.0.6 to 22.0.8 [renovate[bot]]

## 7.1.18 - 2022-08-06

* Update autumndb from 22.0.5 to 22.0.6 [renovate[bot]]

## 7.1.17 - 2022-08-06

* Update autumndb from 22.0.4 to 22.0.5 [renovate[bot]]

## 7.1.16 - 2022-08-06

* Update autumndb from 22.0.2 to 22.0.4 [renovate[bot]]

## 7.1.15 - 2022-08-06

* Update autumndb from 22.0.0 to 22.0.2 [renovate[bot]]

## 7.1.14 - 2022-08-04

* Update autumndb from 21.3.15 to 22.0.0 [renovate[bot]]

## 7.1.13 - 2022-08-04

* Update autumndb from 21.3.14 to 21.3.15 [renovate[bot]]

## 7.1.12 - 2022-08-02

* Update typedoc from 0.23.9 to 0.23.10 [renovate[bot]]

## 7.1.11 - 2022-08-02

* Update autumndb from 21.3.1 to 21.3.14 [renovate[bot]]

## 7.1.10 - 2022-08-02

* Renovate: Increase concurrency to 2 [Josh Bowling]

## 7.1.9 - 2022-07-31

* Update autumndb from 21.2.0 to 21.3.1 [renovate[bot]]

## 7.1.8 - 2022-07-31

* Update ts-jest from 28.0.5 to 28.0.7 [renovate[bot]]

## 7.1.7 - 2022-07-31

* Update lint-staged from 13.0.2 to 13.0.3 [renovate[bot]]

## 7.1.6 - 2022-07-31

* Update jest monorepo [renovate[bot]]

## 7.1.5 - 2022-07-25

* Update autumndb from 21.1.0 to 21.2.0 [renovate[bot]]

## 7.1.4 - 2022-07-24

* Update typedoc from 0.23.8 to 0.23.9 [renovate[bot]]

## 7.1.3 - 2022-07-20

* Update autumndb from 21.0.0 to 21.1.0 [renovate[bot]]

## 7.1.2 - 2022-07-19

* Update autumndb from 20.3.3 to 21.0.0 [renovate[bot]]

## 7.1.1 - 2022-07-18

* Update typedoc from 0.23.7 to 0.23.8 [renovate[bot]]

## 7.1.0 - 2022-07-12

* Run codeQL on PR only once [Lucian Buzzo]
* Add countBy method from lodash [Lucian Buzzo]

## 7.0.26 - 2022-07-09

* Update typedoc from 0.23.6 to 0.23.7 [renovate[bot]]

## 7.0.25 - 2022-07-08

* Update typedoc from 0.23.5 to 0.23.6 [renovate[bot]]

## 7.0.24 - 2022-07-07

* Drop jellyfish-types [Josh Bowling]

## 7.0.23 - 2022-07-02

* Update typedoc from 0.23.3 to 0.23.5 [renovate[bot]]
* Update typedoc from 0.23.2 to 0.23.3 [renovate[bot]]

## 7.0.22 - 2022-06-28

* Update: update typedoc from 0.23.1 to 0.23.2 [renovate[bot]]

## 7.0.21 - 2022-06-27

* Update dependencies (external v0 minor bumps) [renovate[bot]]
* Update typedoc from 0.22.17 to 0.22.18 [renovate[bot]]

## 7.0.20 - 2022-06-19

* Bump dependencies [Josh Bowling]

## 7.0.19 - 2022-06-19

* Drop @balena/jellyfish-config [Josh Bowling]

## 7.0.18 - 2022-06-17

* Update typescript from 4.7.3 to 4.7.4 [renovate[bot]]

## 7.0.17 - 2022-06-05

* Update @types/jest from 27.5.1 to 28.1.0 [Josh Bowling]

## 7.0.16 - 2022-06-05

* Update lint-staged from 12.4.2 to 13.0.0 [Josh Bowling]

## 7.0.15 - 2022-06-03

* Update typescript from 4.7.2 to 4.7.3 [Renovate Bot]

## 7.0.14 - 2022-06-01

* Update typedoc from 0.22.16 to 0.22.17 [Renovate Bot]
* Update typescript from 4.6.4 to 4.7.2 [Renovate Bot]
* Update typedoc from 0.22.15 to 0.22.16 [Renovate Bot]

## 7.0.13 - 2022-05-28

* Update external-non-major (external non-major bumps) [Renovate Bot]

## 7.0.12 - 2022-05-14

* Update external-non-major [Renovate Bot]

## 7.0.11 - 2022-05-07

* Update external-non-major [Renovate Bot]

## 7.0.10 - 2022-05-04

* Bump jest dependencies [Josh Bowling]

## 7.0.9 - 2022-04-28

* Update typescript from 4.6.3 to 4.6.4 [Renovate Bot]

## 7.0.8 - 2022-04-15

* Update internal-patch [Renovate Bot]

## 7.0.7 - 2022-04-15

* 🔄 Synced file(s) with product-os/jellyfish-config [joshbwlng]

## 7.0.6 - 2022-04-12

* Update typedoc from 0.22.14 to 0.22.15 [Renovate Bot]

## 7.0.5 - 2022-04-07

* Update typedoc from 0.22.13 to 0.22.14 [Renovate Bot]

## 7.0.4 - 2022-03-26

* Update external-non-major [Renovate Bot]

## 7.0.3 - 2022-03-26

* Update typescript from 4.6.2 to 4.6.3 [Renovate Bot]

## 7.0.2 - 2022-03-22

* Update upstreams in repo.yml [Josh Bowling]

## 7.0.1 - 2022-03-19

* Update external-non-major [Renovate Bot]

## 7.0.0 - 2022-03-15

* Remove dependency on jellyfish-assert [Lucian Buzzo]
* Remove code for generating triggered actions [Lucian Buzzo]

## 6.1.0 - 2022-03-15

* Add generic functionality for introspecting schemas for object references [Lucian Buzzo]

## 6.0.0 - 2022-03-14

* Drop support for $events syntactic sugar [Lucian Buzzo]
* Drop transformers specific formulas [Lucian Buzzo]
* Add ability to provide custom formulas to parser instance [Lucian Buzzo]
* Use a parser class instead of pure functions [Lucian Buzzo]

## 5.3.0 - 2022-03-11

* Allow AGGREGATE to accept an initial value [Josh Bowling]

## 5.2.2 - 2022-03-11

* evaluate: Return null on missing input [Josh Bowling]
* Downgrade formulajs [Josh Bowling]
* Stop skipping most tests [Josh Bowling]

## 5.2.1 - 2022-03-09

* Update dependencies (external non-major bumps) [Renovate Bot]

## 5.2.0 - 2022-03-09

* Improve the error message thrown by evaluate() [Lucian Buzzo]

## 5.1.51 - 2022-03-09

* Ensure AGGREGATE removes undefined values [Lucian Buzzo]

## 5.1.50 - 2022-03-09

* Make AGGREGATE behave consistently [Lucian Buzzo]

## 5.1.49 - 2022-03-07

* Update typescript from 4.5.5 to 4.6.2 [Renovate Bot]

## 5.1.48 - 2022-03-07

* Update typedoc from 0.22.12 to 0.22.13 [Renovate Bot]

## 5.1.47 - 2022-03-07

* Update internal-patch [Renovate Bot]

## 5.1.46 - 2022-03-05

* Update internal-patch [Renovate Bot]

## 5.1.45 - 2022-03-05

* Update lint-staged from 12.3.4 to 12.3.5 [Josh Bowling]

## 5.1.44 - 2022-03-04

* Create codeql-analysis.yml [Lucian Buzzo]

## 5.1.43 - 2022-02-26

* Update internal-patch [Renovate Bot]

## 5.1.42 - 2022-02-26

* Update external-non-major [Renovate Bot]

## 5.1.41 - 2022-02-22

* Update @balena/jellyfish-client-sdk from 10.0.0 to 10.1.0 [Renovate Bot]

## 5.1.40 - 2022-02-20

* Update dependencies (internal patch bumps) [Renovate Bot]

## 5.1.39 - 2022-02-20

* Update @balena/jellyfish-client-sdk from 8.3.7 to 10.0.0 [Renovate Bot]

## 5.1.38 - 2022-02-20

* Update typedoc from 0.22.11 to 0.22.12 [Renovate Bot]

## 5.1.37 - 2022-02-19

* Update dependencies (external major bumps) [Renovate Bot]

## 5.1.36 - 2022-02-15

* Remove logging for deprecated this.links in expressions [Lucian Buzzo]

## 5.1.35 - 2022-02-15

* Improve README with a better example and description [Lucian Buzzo]

## 5.1.34 - 2022-02-15

* Correctly order formula evaluation in objects [Lucian Buzzo]

## 5.1.33 - 2022-02-15

* Update internal-patch [Renovate Bot]

## 5.1.32 - 2022-02-13

* Update internal-patch [Renovate Bot]

## 5.1.31 - 2022-02-12

* Update internal-patch [Renovate Bot]

## 5.1.30 - 2022-02-08

* Update external-non-major [Renovate Bot]

## 5.1.29 - 2022-02-05

* Update internal-patch [Renovate Bot]

## 5.1.28 - 2022-02-05

* Update internal-patch [Renovate Bot]

## 5.1.27 - 2022-02-03

* Minor fixes [Carol Schulze]

## 5.1.26 - 2022-02-03

* Update @balena/jellyfish-client-sdk from 8.1.2 to 8.2.0 [Renovate Bot]

## 5.1.25 - 2022-01-31

* Update internal-patch [Renovate Bot]

## 5.1.24 - 2022-01-29

* Update dependencies (external non-major bumps) [Renovate Bot]

## 5.1.23 - 2022-01-29

* Update dependencies (internal patch bumps) [Renovate Bot]

## 5.1.22 - 2022-01-25

* patch: Update dependency @balena/jellyfish-client-sdk to ^8.1.0 [Renovate Bot]

## 5.1.21 - 2022-01-24

* patch: Update dependency @balena/jellyfish-logger to ^4.0.27 [Renovate Bot]

## 5.1.20 - 2022-01-23

* patch: Update internal-non-major [Renovate Bot]

## 5.1.19 - 2022-01-23

* patch: Update dependency lint-staged to ^12.3.1 [Renovate Bot]

## 5.1.18 - 2022-01-23

* patch: Update internal-non-major [Renovate Bot]

## 5.1.17 - 2022-01-23

* patch: Update dependency @balena/jellyfish-logger to ^4.0.24 [Renovate Bot]

## 5.1.16 - 2022-01-23

* patch: Update dependency @balena/jellyfish-assert to ^1.2.10 [Renovate Bot]

## 5.1.15 - 2022-01-23

* patch: Update dependency lint-staged to ^12.3.0 [Renovate Bot]

## 5.1.14 - 2022-01-23

* patch: Update dependency @balena/jellyfish-client-sdk to ^8.0.2 [Renovate Bot]

## 5.1.13 - 2022-01-22

* patch: Update dependency @balena/jellyfish-logger to ^4.0.22 [Renovate Bot]

## 5.1.12 - 2022-01-22

* patch: Update dependency @balena/jellyfish-client-sdk to v8 [Renovate Bot]

## 5.1.11 - 2022-01-22

* patch: Update internal-non-major [Renovate Bot]

## 5.1.10 - 2022-01-22

* patch: Update external-non-major [Renovate Bot]

## 5.1.9 - 2022-01-19

* patch: Update dependency @balena/jellyfish-logger to ^4.0.19 [Renovate Bot]

## 5.1.8 - 2022-01-19

* patch: Update internal-non-major [Renovate Bot]
* patch: Update dependency typedoc to ^0.22.11 [Renovate Bot]

## 5.1.7 - 2022-01-16

* patch: Update dependency @balena/jellyfish-logger to ^4.0.17 [Renovate Bot]

## 5.1.6 - 2022-01-15

* patch: Update dependency @balena/jellyfish-types to ^2.0.4 [Renovate Bot]

## 5.1.5 - 2022-01-15

* patch: Update internal-patch [Renovate Bot]

## 5.1.4 - 2022-01-15

* patch: Update dependency ts-jest to ^27.1.3 [Renovate Bot]

## 5.1.3 - 2022-01-13

* patch: Update dependency @balena/jellyfish-client-sdk to ^7.0.36 [Renovate Bot]

## 5.1.2 - 2022-01-12

* patch: Update dependency @balena/jellyfish-client-sdk to ^7.0.35 [Renovate Bot]

## 5.1.1 - 2022-01-10

* patch: Update dependency @balena/jellyfish-client-sdk to ^7.0.34 [Renovate Bot]

## 5.1.0 - 2022-01-10

* Add needs formula [stathismor]

## 5.0.5 - 2022-01-08

* patch: Update dependency @balena/jellyfish-logger to ^4.0.15 [Renovate Bot]

## 5.0.4 - 2022-01-08

* patch: Update internal-patch [Renovate Bot]

## 5.0.3 - 2022-01-08

* patch: Update external-non-major [Renovate Bot]

## 5.0.2 - 2022-01-08

* patch: Update internal-patch [Renovate Bot]

## 5.0.1 - 2022-01-05

* patch: Update dependency @balena/jellyfish-types to ^2.0.2 [Renovate Bot]

## 5.0.0 - 2022-01-04

* Clean up types [Josh Bowling]

## 4.9.174 - 2022-01-03

* patch: Update dependency @balena/jellyfish-logger to ^4.0.12 [Renovate Bot]

## 4.9.173 - 2022-01-03

* patch: Update dependency @balena/jellyfish-logger to ^4.0.11 [Renovate Bot]

## 4.9.172 - 2022-01-02

* patch: Update internal-patch [Renovate Bot]

## 4.9.171 - 2022-01-02

* patch: Update dependency lint-staged to ^12.1.5 [Renovate Bot]

## 4.9.170 - 2022-01-01

* patch: Update internal-patch [Renovate Bot]

## 4.9.169 - 2022-01-01

* patch: Update internal-patch [Renovate Bot]

## 4.9.168 - 2021-12-30

* patch: Update external-non-major [Renovate Bot]

## 4.9.167 - 2021-12-25

* patch: Update internal-patch [Renovate Bot]

## 4.9.166 - 2021-12-25

* patch: Update internal-patch [Renovate Bot]

## 4.9.165 - 2021-12-19

* patch: Update internal-patch [Renovate Bot]

## 4.9.164 - 2021-12-18

* patch: Update internal-patch [Renovate Bot]

## 4.9.163 - 2021-12-18

* patch: Update dependency lint-staged to ^12.1.3 [Renovate Bot]

## 4.9.162 - 2021-12-18

* patch: Update internal-patch [Renovate Bot]

## 4.9.161 - 2021-12-18

* patch: Update external-non-major [Renovate Bot]

## 4.9.160 - 2021-12-17

* patch: Update dependency @balena/jellyfish-assert to ^1.2.0 [Renovate Bot]

## 4.9.159 - 2021-12-17

* patch: Update dependency @balena/jellyfish-logger to v4 [Renovate Bot]

## 4.9.158 - 2021-12-14

* patch: Update internal-patch [Renovate Bot]

## 4.9.157 - 2021-12-14

* Drop depcheck, bump typescript [Josh Bowling]

## 4.9.156 - 2021-12-11

* patch: Update dependency @balena/jellyfish-client-sdk to ^7.0.25 [Renovate Bot]

## 4.9.155 - 2021-12-11

* patch: Update internal-patch [Renovate Bot]

## 4.9.154 - 2021-12-11

* patch: Update dependency @balena/jellyfish-logger to ^3.0.164 [Renovate Bot]

## 4.9.153 - 2021-12-11

* patch: Update internal-patch [Renovate Bot]

## 4.9.152 - 2021-12-11

* patch: Update external-non-major [Renovate Bot]
* patch: Update dependency typescript to ^4.5.3 [Renovate Bot]
* patch: Update dependency @balena/jellyfish-logger to ^3.0.163 [Renovate Bot]

## 4.9.151 - 2021-12-07

* patch: Update dependency @balena/jellyfish-logger to ^3.0.162 [Renovate Bot]

## 4.9.150 - 2021-12-05

* patch: Update dependency @balena/jellyfish-assert to ^1.1.101 [Renovate Bot]

## 4.9.149 - 2021-12-05

* patch: Update internal-patch [Renovate Bot]

## 4.9.148 - 2021-12-05

* patch: Update dependency ts-jest to ^27.1.0 [Renovate Bot]

## 4.9.147 - 2021-12-04

* patch: Update dependency @balena/jellyfish-types to ^1.2.37 [Renovate Bot]

## 4.9.146 - 2021-12-04

* patch: Update internal-patch [Renovate Bot]

## 4.9.145 - 2021-12-04

* patch: Update dependency @balena/jellyfish-client-sdk to ^7.0.22 [Renovate Bot]

## 4.9.144 - 2021-12-04

* patch: Update dependency jest to ^27.4.3 [Renovate Bot]

## 4.9.143 - 2021-12-01

* patch: Update dependency @balena/jellyfish-logger to ^3.0.156 [Renovate Bot]

## 4.9.142 - 2021-11-29

* patch: Update dependency @balena/jellyfish-logger to ^3.0.155 [Renovate Bot]

## 4.9.141 - 2021-11-29

* patch: Update internal-patch [Renovate Bot]

## 4.9.140 - 2021-11-27

* patch: Update internal-patch [Renovate Bot]

## 4.9.139 - 2021-11-27

* patch: Update dependency @balena/jellyfish-config to v2 [Renovate Bot]

## 4.9.138 - 2021-11-27

* patch: Update internal-patch [Renovate Bot]

## 4.9.137 - 2021-11-26

* patch: Update internal-patch [Renovate Bot]

## 4.9.136 - 2021-11-26

* patch: Update dependency @balena/lint to ^6.2.0 [Renovate Bot]

## 4.9.135 - 2021-11-26

* patch: Update internal-patch [Renovate Bot]

## 4.9.134 - 2021-11-26

* Use product-os/renovate-config [Josh Bowling]

## 4.9.133 - 2021-11-25

* patch: Update internal-patch [Renovate Bot]

## 4.9.132 - 2021-11-25

* Bump typedoc to v0.22.10 [Josh Bowling]
* patch: Update dependency typescript to ^4.5.2 [Renovate Bot]

## 4.9.131 - 2021-11-25

* patch: Update internal-patch [Renovate Bot]

## 4.9.130 - 2021-11-25

* Remove outdated upstreams from repo.yml [Josh Bowling]

## 4.9.129 - 2021-11-25

* Remove jellycheck [Josh Bowling]

## 4.9.128 - 2021-11-24

* patch: Update internal-patch [Renovate Bot]

## 4.9.127 - 2021-11-24

* patch: Update internal-patch [Renovate Bot]

## 4.9.126 - 2021-11-24

* Unpin typescript [Josh Bowling]

## 4.9.125 - 2021-11-24

* patch: Update internal-patch [Renovate Bot]

## 4.9.124 - 2021-11-24

* patch: Update internal-patch [Renovate Bot]

## 4.9.123 - 2021-11-24

* Update depcheck ignore list [Josh Bowling]
* Bring back typedoc, pin typescript [Josh Bowling]

## 4.9.122 - 2021-11-24

* Move typedoc to docs action [Josh Bowling]

## 4.9.121 - 2021-11-24

* patch: Update dependency lint-staged to ^12.1.2 [Renovate Bot]

## 4.9.120 - 2021-11-24

* patch: Update internal-patch [Renovate Bot]

## 4.9.119 - 2021-11-22

* patch: Update internal-patch [Renovate Bot]

## 4.9.118 - 2021-11-22

* patch: Update internal-patch [Renovate Bot]

## 4.9.117 - 2021-11-22

* Bump external deps [Josh Bowling]

## 4.9.116 - 2021-11-21

* patch: Update internal-patch [Renovate Bot]

## 4.9.115 - 2021-11-21

* patch: Update internal-patch [Renovate Bot]
* patch: Update dependency @balena/jellyfish-logger to ^3.0.130 [Renovate Bot]

## 4.9.114 - 2021-11-20

* patch: Update dependency @balena/jellyfish-logger to ^3.0.126 [Renovate Bot]

## 4.9.113 - 2021-11-20

* Remove license headers [Josh Bowling]
* patch: Update internal-patch [Renovate Bot]

## 4.9.112 - 2021-11-20

* patch: Update dependency @balena/jellycheck to v1 [Renovate Bot]

## 4.9.111 - 2021-11-16

* patch: Update dependency @balena/jellyfish-logger to ^3.0.124 [Renovate Bot]

## 4.9.110 - 2021-11-16

* patch: Update internal-patch [Renovate Bot]

## 4.9.109 - 2021-11-16

* patch: Update internal-patch [Renovate Bot]

## 4.9.108 - 2021-11-16

* 🔄 Synced file(s) with product-os/jellyfish-config [joshbwlng]

## 4.9.107 - 2021-11-16

* patch: Update internal-patch [Renovate Bot]

## 4.9.106 - 2021-11-15

* patch: Update dependency @balena/jellyfish-logger to ^3.0.120 [Renovate Bot]

## 4.9.105 - 2021-11-15

* patch: Update internal-patch [Renovate Bot]

## 4.9.104 - 2021-11-15

* patch: Update dependency @balena/jellyfish-logger to ^3.0.117 [Renovate Bot]

## 4.9.103 - 2021-11-15

* patch: Update internal-patch [Renovate Bot]

## 4.9.102 - 2021-11-15

* Set license [Josh Bowling]

## 4.9.101 - 2021-11-15

* patch: Update dependency typedoc to ^0.22.9 [Renovate Bot]

## 4.9.100 - 2021-11-14

* patch: Update dependency lint-staged to v12 [Renovate Bot]

## 4.9.99 - 2021-11-14

* patch: Update dependency @balena/jellyfish-logger to ^3.0.112 [Renovate Bot]

## 4.9.98 - 2021-11-14

* patch: Update dependency @balena/jellyfish-logger to ^3.0.111 [Renovate Bot]

## 4.9.97 - 2021-11-14

* patch: Update dependency @balena/jellyfish-assert to ^1.1.75 [Renovate Bot]

## 4.9.96 - 2021-11-14

* patch: Update dependency @balena/jellyfish-types to ^1.2.15 [Renovate Bot]

## 4.9.95 - 2021-11-14

* patch: Update internal-patch [Renovate Bot]

## 4.9.94 - 2021-11-13

* patch: Update internal-patch [Renovate Bot]

## 4.9.93 - 2021-11-13

* patch: Update dependency @balena/jellyfish-assert to ^1.1.72 [Renovate Bot]

## 4.9.92 - 2021-11-13

* patch: Update internal-patch [Renovate Bot]

## 4.9.91 - 2021-11-12

* patch: Update dependency @balena/jellyfish-logger to ^3.0.107 [Renovate Bot]

## 4.9.90 - 2021-11-11

* patch: Update dependency @balena/jellyfish-logger to ^3.0.105 [Renovate Bot]

## 4.9.89 - 2021-11-11

* patch: Update dependency @balena/jellyfish-assert to ^1.1.71 [Renovate Bot]

## 4.9.88 - 2021-11-11

* patch: Update internal-patch [Renovate Bot]

## 4.9.87 - 2021-11-10

* Update example usage in readme [Josh Bowling]

## 4.9.86 - 2021-11-09

* patch: Update dependency @balena/jellyfish-logger to ^3.0.104 [Renovate Bot]

## 4.9.85 - 2021-11-08

* patch: Update dependency @balena/jellyfish-config to ^1.5.8 [Renovate Bot]

## 4.9.84 - 2021-11-07

* patch: Update internal-patch [Renovate Bot]

## 4.9.83 - 2021-11-07

* patch: Update dependency typedoc to ^0.22.8 [Renovate Bot]

## 4.9.82 - 2021-11-06

* patch: Update internal-patch [Renovate Bot]

## 4.9.81 - 2021-11-05

* patch: Update dependency @balena/jellyfish-logger to ^3.0.100 [Renovate Bot]

## 4.9.80 - 2021-11-05

* patch: Update dependency @balena/jellyfish-config to ^1.5.7 [Renovate Bot]

## 4.9.79 - 2021-11-03

* created triggered actions with versions matching the type [Martin Rauscher]

## 4.9.78 - 2021-10-31

* patch: Update dependency @balena/jellyfish-client-sdk to v7 [Renovate Bot]

## 4.9.77 - 2021-10-30

* patch: Update internal-patch [Renovate Bot]

## 4.9.76 - 2021-10-30

* patch: Update external-non-major [Renovate Bot]

## 4.9.75 - 2021-10-29

* patch: Update dependency @balena/jellyfish-client-sdk to ^6.0.3 [Renovate Bot]

## 4.9.74 - 2021-10-28

* patch: Update internal-patch [Renovate Bot]

## 4.9.73 - 2021-10-28

* patch: Update dependency @balena/jellyfish-config to ^1.5.6 [Renovate Bot]

## 4.9.72 - 2021-10-28

* patch: Update internal-patch [Renovate Bot]

## 4.9.71 - 2021-10-28

* patch: Update dependency @balena/jellyfish-types to ^1.2.8 [Renovate Bot]

## 4.9.70 - 2021-10-28

* patch: Update internal-patch [Renovate Bot]

## 4.9.69 - 2021-10-25

* fix: link formulas didn't trigger on never-updated contracts [Martin Rauscher]

## 4.9.68 - 2021-10-23

* patch: Update internal-patch [Renovate Bot]

## 4.9.67 - 2021-10-23

* patch: Update dependency lint-staged to ^11.2.4 [Renovate Bot]

## 4.9.66 - 2021-10-23

* patch: Update dependency @balena/jellyfish-logger to ^3.0.91 [Renovate Bot]

## 4.9.65 - 2021-10-23

* patch: Update dependency @balena/jellyfish-logger to ^3.0.90 [Renovate Bot]

## 4.9.64 - 2021-10-23

* patch: Update internal-patch [Renovate Bot]

## 4.9.63 - 2021-10-23

* patch: Update external-non-major [Renovate Bot]

## 4.9.62 - 2021-10-20

* patch: Update dependency @balena/jellyfish-logger to ^3.0.87 [Renovate Bot]

## 4.9.61 - 2021-10-19

* patch: Update internal-patch [Renovate Bot]

## 4.9.60 - 2021-10-04

* 🔄 Synced file(s) with product-os/jellyfish-config [joshbwlng]

## 4.9.59 - 2021-10-04

* patch: Update internal-patch [Renovate Bot]

## 4.9.58 - 2021-10-04

* patch: Update dependency @balena/jellyfish-config to ^1.5.1 [Renovate Bot]

## 4.9.57 - 2021-10-02

* patch: Update dependency @balena/jellyfish-logger to ^3.0.76 [Renovate Bot]

## 4.9.56 - 2021-10-02

* patch: Update dependency typedoc to ^0.22.5 [Renovate Bot]

## 4.9.55 - 2021-10-02

* patch: Update dependency jest to ^27.2.4 [Renovate Bot]

## 4.9.54 - 2021-10-02

* patch: Update dependency @balena/jellyfish-logger to ^3.0.75 [Renovate Bot]

## 4.9.53 - 2021-09-25

* patch: Update internal-patch [Renovate Bot]

## 4.9.52 - 2021-09-25

* patch: Update dependency jest to ^27.2.2 [Renovate Bot]

## 4.9.51 - 2021-09-25

* patch: Update dependency @balena/jellyfish-logger to ^3.0.73 [Renovate Bot]

## 4.9.50 - 2021-09-25

* patch: Update dependency @balena/jellyfish-logger to ^3.0.72 [Renovate Bot]

## 4.9.49 - 2021-09-25

* Add typedoc to depcheck ignore list [Josh Bowling]
* patch: Update external-non-major [Renovate Bot]

## 4.9.48 - 2021-09-22

* patch: Update dependency @balena/jellyfish-types to ^1.2.1 [Renovate Bot]

## 4.9.47 - 2021-09-21

* Bump jellyfish-types to v1.2.0 [Josh Bowling]

## 4.9.46 - 2021-09-20

* patch: Update dependency @balena/jellyfish-logger to ^3.0.71 [Renovate Bot]

## 4.9.45 - 2021-09-20

* ensure triggered actions for formulas run on any change [Martin Rauscher]

## 4.9.44 - 2021-09-06

* patch: Update dependency @balena/jellyfish-types to v1 [Renovate Bot]

## 4.9.43 - 2021-09-04

* patch: Update dependency @balena/jellyfish-logger to ^3.0.67 [Renovate Bot]

## 4.9.42 - 2021-08-30

* patch: Update dependency @balena/jellyfish-logger to ^3.0.66 [Renovate Bot]

## 4.9.41 - 2021-08-30

* patch: Update dependency typedoc to ^0.21.9 [Renovate Bot]

## 4.9.40 - 2021-08-29

* Bump typedoc to v0.21.8 [Josh Bowling]
* Add groupName to formulajs Renovate group [Josh Bowling]

## 4.9.39 - 2021-08-28

* Add formulajs Renovate group [Josh Bowling]

## 4.9.38 - 2021-08-28

* patch: Update dependency @balena/jellyfish-logger to ^3.0.65 [Renovate Bot]

## 4.9.37 - 2021-08-28

* Bump external dependencies [Josh Bowling]

## 4.9.36 - 2021-08-27

* Bump typescript to v4.4.2 [Josh Bowling]

## 4.9.35 - 2021-08-24

* patch: Update dependency @balena/jellyfish-client-sdk to v6 [Renovate Bot]

## 4.9.34 - 2021-08-23

* patch: add test for string concatenation with CONCATENATE function and + operator [Thomas Manning]

## 4.9.33 - 2021-08-21

* patch: Update dependency @balena/jellyfish-logger to ^3.0.64 [Renovate Bot]

## 4.9.32 - 2021-08-21

* patch: Update internal-patch [Renovate Bot]

## 4.9.31 - 2021-08-20

* patch: Update dependency @balena/jellyfish-logger to ^3.0.61 [Renovate Bot]

## 4.9.30 - 2021-08-19

* Bump typedoc to v0.21.6 [Josh Bowling]

## 4.9.29 - 2021-08-19

* patch: Update dependency @balena/jellyfish-types to ^0.9.8 [Renovate Bot]

## 4.9.28 - 2021-08-18

* Bump external dependencies [Josh Bowling]

## 4.9.27 - 2021-08-18

* patch: Update dependency @balena/jellyfish-types to ^0.9.7 [Renovate Bot]

## 4.9.26 - 2021-08-18

* patch: Update dependency @balena/jellyfish-types to ^0.9.6 [Renovate Bot]

## 4.9.25 - 2021-08-17

* patch: Update dependency @balena/jellyfish-types to ^0.9.5 [Renovate Bot]

## 4.9.24 - 2021-08-16

* Bump fast-json-patch to v3.1.0 [Josh Bowling]

## 4.9.23 - 2021-08-16

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.5.2 [Renovate Bot]

## 4.9.22 - 2021-08-16

* patch: Update dependency @balena/jellyfish-logger to ^3.0.60 [Renovate Bot]

## 4.9.21 - 2021-08-15

* patch: Update dependency @types/jest to v27 [Renovate Bot]

## 4.9.20 - 2021-08-15

* patch: Update dependency @balena/jellyfish-logger to ^3.0.59 [Renovate Bot]

## 4.9.19 - 2021-08-10

* patch: Update dependency @balena/jellyfish-types to ^0.9.4 [Renovate Bot]

## 4.9.18 - 2021-08-10

* patch: Update dependency @balena/jellyfish-types to ^0.9.2 [Renovate Bot]

## 4.9.17 - 2021-08-09

* patch: Update dependency @balena/jellyfish-logger to ^3.0.58 [Renovate Bot]

## 4.9.16 - 2021-08-09

* patch: Update dependency @balena/jellyfish-types to ^0.9.0 [Renovate Bot]

## 4.9.15 - 2021-08-07

* patch: Update internal-patch [Renovate Bot]

## 4.9.14 - 2021-08-06

* Bump external non-major deps [Josh Bowling]

## 4.9.13 - 2021-08-06

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.5.1 [Renovate Bot]

## 4.9.12 - 2021-08-06

* patch: Update dependency @balena/jellyfish-logger to ^3.0.56 [Renovate Bot]

## 4.9.11 - 2021-08-05

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.5.0 [Renovate Bot]

## 4.9.10 - 2021-08-05

* Remove unused schedule field from triggered-actions [Lucian Buzzo]

## 4.9.9 - 2021-08-05

* patch: Update internal-patch [Renovate Bot]

## 4.9.8 - 2021-08-04

* use balena convention for npm script names [Martin Rauscher]

## 4.9.7 - 2021-08-01

* Bump ts-jest to v27.0.4 [Josh Bowling]

## 4.9.6 - 2021-08-01

* Bump typedoc to v0.21.5 [Josh Bowling]

## 4.9.5 - 2021-08-01

* Bump lint-staged to v11.1.1 [Josh Bowling]

## 4.9.4 - 2021-07-31

* patch: Update dependency @balena/jellyfish-config to ^1.4.8 [Renovate Bot]

## 4.9.3 - 2021-07-30

* patch: Update dependency @balena/jellycheck to ^0.1.3 [Renovate Bot]

## 4.9.2 - 2021-07-29

* patch: Update dependency @balena/jellyfish-config to ^1.4.7 [Renovate Bot]

## 4.9.1 - 2021-07-28

* Check for event link contraints when creating link triggers [Graham McCulloch]
* Remove 'this' context variable [Graham McCulloch]

## 4.9.0 - 2021-07-27

* Make 'this' context object also available as 'contract'. [Graham McCulloch]

## 4.8.1 - 2021-07-26

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.4.1 [Renovate Bot]

## 4.8.0 - 2021-07-26

* Add LAST and ORDER_BY formulajs functions [Graham McCulloch]
* Temp: pin formulajs dependency to 2.6.10 Version 2.7 has a breaking change wrt how undefined/null inputs are handled. [Graham McCulloch]

## 4.7.1 - 2021-07-21

* patch: Update dependency @balena/jellyfish-config to ^1.4.6 [Renovate Bot]

## 4.7.0 - 2021-07-20

* fix triggered actions for updating linked formulas [Martin Rauscher]

## 4.6.13 - 2021-07-19

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.4.0 [Renovate Bot]

## 4.6.12 - 2021-07-17

* patch: Update dependency @balena/jellyfish-assert to ^1.1.66 [Renovate Bot]

## 4.6.11 - 2021-07-17

* patch: Update external-non-major [Renovate Bot]

## 4.6.10 - 2021-07-17

* patch: Update dependency @balena/jellyfish-types to ^0.8.12 [Renovate Bot]

## 4.6.9 - 2021-07-17

* patch: Update dependency @balena/jellyfish-types to ^0.8.11 [Renovate Bot]

## 4.6.8 - 2021-07-17

* patch: Update internal-patch [Renovate Bot]

## 4.6.7 - 2021-07-16

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.3.4 [Renovate Bot]

## 4.6.6 - 2021-07-16

* Remove whitesource config [Josh Bowling]

## 4.6.5 - 2021-07-14

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.3.3 [Renovate Bot]

## 4.6.4 - 2021-07-14

* patch: Update dependency @balena/jellyfish-types to ^0.8.10 [Renovate Bot]

## 4.6.3 - 2021-07-14

* patch: Update dependency @balena/jellyfish-types to ^0.8.9 [Renovate Bot]

## 4.6.2 - 2021-07-14

* Fix jest config [Graham McCulloch]

## 4.6.1 - 2021-07-14

* patch: Update internal-patch [Renovate Bot]

## 4.6.0 - 2021-07-14

* Exclude test folder from jest roots [Graham McCulloch]
* Add support for a REJECT formulajs method [Graham McCulloch]

## 4.5.0 - 2021-07-13

* Add unit test for combination of formulajs functions [Graham McCulloch]
* Add support for a FILTER formulajs method [Graham McCulloch]

## 4.4.9 - 2021-07-11

* patch: Update dependency @balena/jellyfish-types to ^0.8.8 [Renovate Bot]

## 4.4.8 - 2021-07-11

* patch: Update dependency @balena/jellyfish-assert to ^1.1.63 [Renovate Bot]

## 4.4.7 - 2021-07-10

* patch: Update dependency @balena/jellyfish-types to ^0.8.7 [Renovate Bot]

## 4.4.6 - 2021-07-10

* patch: Update dependency typedoc to ^0.21.3 [Renovate Bot]

## 4.4.5 - 2021-07-10

* patch: Update internal-patch [Renovate Bot]

## 4.4.4 - 2021-07-10

* patch: Update external-non-major [Renovate Bot]

## 4.4.3 - 2021-07-09

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.3.1 [Renovate Bot]

## 4.4.2 - 2021-07-09

* patch: Update internal-patch [Renovate Bot]

## 4.4.1 - 2021-07-06

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.3.0 [Renovate Bot]

## 4.4.0 - 2021-07-06

* Add VALUES formula [Stathis]

## 4.3.10 - 2021-07-06

* fix link triggers in case SDK has no known links [Martin Rauscher]

## 4.3.9 - 2021-07-05

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.10 [Renovate Bot]

## 4.3.8 - 2021-07-05

* patch: Update internal-patch [Renovate Bot]

## 4.3.7 - 2021-07-04

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.9 [Renovate Bot]

## 4.3.6 - 2021-07-03

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.8 [Renovate Bot]

## 4.3.5 - 2021-07-03

* patch: Update internal-patch [Renovate Bot]

## 4.3.4 - 2021-07-03

* patch: Update internal-patch [Renovate Bot]

## 4.3.3 - 2021-07-03

* patch: Update dependency typescript to ^4.3.5 [Renovate Bot]

## 4.3.2 - 2021-07-02

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.5 [Renovate Bot]

## 4.3.1 - 2021-07-02

* patch: Update internal-patch [Renovate Bot]

## 4.3.0 - 2021-07-02

* Add SOME method to formulas [Lucian Buzzo]

## 4.2.32 - 2021-07-02

* patch: Update internal-patch [Renovate Bot]

## 4.2.31 - 2021-07-02

* patch: Update dependency @balena/jellyfish-types to ^0.8.0 [Renovate Bot]

## 4.2.30 - 2021-07-02

* patch: Update dependency @balena/jellyfish-config to ^1.4.0 [Renovate Bot]

## 4.2.29 - 2021-06-30

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.3 [Renovate Bot]

## 4.2.28 - 2021-06-30

* patch: Update dependency @balena/jellyfish-assert to ^1.1.55 [Renovate Bot]

## 4.2.27 - 2021-06-30

* 🔄 Synced file(s) with product-os/jellyfish-config [jellyfish-bot]

## 4.2.26 - 2021-06-29

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.2.2 [Renovate Bot]

## 4.2.25 - 2021-06-29

* patch: Update dependency @balena/jellyfish-types to ^0.7.3 [Renovate Bot]

## 4.2.24 - 2021-06-29

* patch: Update internal-patch [Renovate Bot]

## 4.2.23 - 2021-06-29

* patch: Update external-non-major [Renovate Bot]

## 4.2.22 - 2021-06-25

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.1.1 [Renovate Bot]

## 4.2.21 - 2021-06-25

* Bump depcheck to v1.4.2 [Josh Bowling]

## 4.2.20 - 2021-06-25

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.1.0 [Renovate Bot]

## 4.2.19 - 2021-06-24

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.9 [Renovate Bot]

## 4.2.18 - 2021-06-24

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.8 [Renovate Bot]

## 4.2.17 - 2021-06-23

* patch: Update dependency @balena/jellyfish-assert to ^1.1.51 [Renovate Bot]

## 4.2.16 - 2021-06-23

* Bump jest to v27.0.5 [Josh Bowling]

## 4.2.15 - 2021-06-23

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.7 [Renovate Bot]

## 4.2.14 - 2021-06-22

* patch: Update dependency @balena/jellyfish-types to ^0.7.0 [Renovate Bot]

## 4.2.13 - 2021-06-22

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.6 [Renovate Bot]

## 4.2.12 - 2021-06-22

* patch: Update internal-patch [Renovate Bot]

## 4.2.11 - 2021-06-19

* patch: Update internal-patch [Renovate Bot]

## 4.2.10 - 2021-06-19

* Bump external non-major deps [Josh Bowling]

## 4.2.9 - 2021-06-19

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.4 [Renovate Bot]

## 4.2.8 - 2021-06-19

* patch: Update dependency @balena/jellyfish-types to ^0.6.10 [Renovate Bot]

## 4.2.7 - 2021-06-15

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.3 [Renovate Bot]

## 4.2.6 - 2021-06-15

* patch: Update internal-patch [Renovate Bot]

## 4.2.5 - 2021-06-15

* Remove package-lock.json [Josh Bowling]
* 🔄 Synced file(s) with product-os/jellyfish-config [jellyfish-bot]

## 4.2.4 - 2021-06-15

* patch: Update dependency @balena/jellyfish-assert to ^1.1.47 [Renovate Bot]

## 4.2.3 - 2021-06-12

* patch: Update dependency @balena/jellyfish-client-sdk to ^5.0.1 [Renovate Bot]

## 4.2.2 - 2021-06-12

* patch: Update dependency @balena/jellyfish-types to ^0.6.8 [Renovate Bot]

## 4.2.1 - 2021-06-12

* patch: Update dependency @balena/jellyfish-client-sdk to v5 [Renovate Bot]

## 4.2.0 - 2021-06-11

* add support for EVERY in formulas [Martin Rauscher]
* cleanup and get rid of utils.ts [Martin Rauscher]

## 4.1.3 - 2021-06-11

* patch: Update internal-patch [Renovate Bot]

## 4.1.2 - 2021-06-11

* patch: Update internal-patch [Renovate Bot]

## 4.1.1 - 2021-06-10

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.7.0 [Renovate Bot]

## 4.1.0 - 2021-06-09

* fix formulas returning 'false' [Martin Rauscher]

## 4.0.18 - 2021-06-08

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.6.0 [Renovate Bot]

## 4.0.17 - 2021-06-08

* patch: Update internal-patch [Renovate Bot]

## 4.0.16 - 2021-06-08

* patch: Update internal-patch [Renovate Bot]

## 4.0.15 - 2021-06-08

* patch: Update dependency @balena/jellyfish-config to ^1.3.0 [Renovate Bot]

## 4.0.14 - 2021-06-08

* patch: Update dependency @balena/jellyfish-config to ^1.2.8 [Renovate Bot]

## 4.0.13 - 2021-06-07

* patch: Update internal-patch [Renovate Bot]

## 4.0.12 - 2021-06-06

* patch: Update internal-patch [Renovate Bot]

## 4.0.11 - 2021-06-06

* patch: Update dependency ts-jest to ^27.0.3 [Renovate Bot]

## 4.0.10 - 2021-06-06

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.4.23 [Renovate Bot]

## 4.0.9 - 2021-06-06

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.4.22 [Renovate Bot]

## 4.0.8 - 2021-06-05

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.4.21 [Renovate Bot]

## 4.0.7 - 2021-06-05

* patch: Update dependency @balena/jellyfish-assert to ^1.1.42 [Renovate Bot]

## 4.0.6 - 2021-06-05

* patch: Update internal-patch [Renovate Bot]

## 4.0.5 - 2021-06-05

* patch: Update dependency @balena/jellyfish-types to ^0.6.1 [Renovate Bot]

## 4.0.4 - 2021-06-05

* patch: Update external-non-major [Renovate Bot]

## 4.0.3 - 2021-06-03

* Improve lint-staged config [Josh Bowling]

## 4.0.2 - 2021-06-02

* Remove unnecessary catch-uncommitted [Josh Bowling]

## 4.0.1 - 2021-05-31

* patch: Update dependency @balena/jellyfish-client-sdk to ^4.4.17 [Renovate Bot]

## 4.0.0 - 2021-05-31

* improve trigger query performancy by constraining on type, if possible [Martin Rauscher]
* add triggered actions for uses of this.links[...] in formulas [Martin Rauscher]
* add support for this.links[...] [Martin Rauscher]

## 3.0.229 - 2021-05-31

* patch: Update dependency @balena/jellyfish-types to ^0.5.137 [Renovate Bot]

## 3.0.228 - 2021-05-31

* patch: Update dependency @balena/jellyfish-config to ^1.2.7 [Renovate Bot]

## 3.0.227 - 2021-05-31

* patch: Update internal-patch [Renovate Bot]

## 3.0.226 - 2021-05-29

* patch: Update dependency @balena/jellyfish-assert to ^1.1.37 [Renovate Bot]

## 3.0.225 - 2021-05-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.134 [Renovate Bot]

## 3.0.224 - 2021-05-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.133 [Renovate Bot]

## 3.0.223 - 2021-05-29

* patch: Update dependency jest to ^27.0.3 [Renovate Bot]

## 3.0.222 - 2021-05-29

* patch: Update dependency @balena/jellyfish-assert to ^1.1.36 [Renovate Bot]

## 3.0.221 - 2021-05-29

* patch: Update dependency jest to ^27.0.2 [Renovate Bot]

## 3.0.220 - 2021-05-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.132 [Renovate Bot]

## 3.0.219 - 2021-05-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.131 [Renovate Bot]

## 3.0.218 - 2021-05-29

* patch: Update dependency @balena/jellyfish-assert to ^1.1.35 [Renovate Bot]

## 3.0.217 - 2021-05-29

* patch: Update external-major to v27 [Renovate Bot]

## 3.0.216 - 2021-05-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.130 [Renovate Bot]

## 3.0.215 - 2021-05-29

* patch: Update dependency @balena/jellyfish-assert to ^1.1.34 [Renovate Bot]

## 3.0.214 - 2021-05-29

* patch: Update external-non-major [Renovate Bot]

## 3.0.213 - 2021-05-27

* patch: Update internal-patch [Renovate Bot]

## 3.0.212 - 2021-05-27

* patch: Update dependency @balena/jellyfish-config to ^1.2.5 [Renovate Bot]

## 3.0.211 - 2021-05-27

* patch: Update internal-patch [Renovate Bot]

## 3.0.210 - 2021-05-27

* 🔄 Synced file(s) with product-os/jellyfish-config [jellyfish-bot]

## 3.0.209 - 2021-05-27

* patch: Update internal-patch [Renovate Bot]

## 3.0.208 - 2021-05-27

* patch: Update internal-patch [Renovate Bot]

## 3.0.207 - 2021-05-27

* patch: Update dependency @balena/jellyfish-assert to ^1.1.28 [Renovate Bot]

## 3.0.206 - 2021-05-27

* patch: Update dependency @balena/jellyfish-config to ^1.2.1 [Renovate Bot]

## 3.0.205 - 2021-05-25

* build(deps): bump browserslist from 4.16.3 to 4.16.6 [dependabot[bot]]

## 3.0.204 - 2021-05-25

* patch: Update dependency @balena/jellyfish-types to ^0.5.124 [Renovate Bot]

## 3.0.203 - 2021-05-25

* patch: Update internal-patch [Renovate Bot]

## 3.0.202 - 2021-05-24

* patch: Update dependency @balena/lint to ^6.1.1 [Renovate Bot]

## 3.0.201 - 2021-05-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.121 [Renovate Bot]

## 3.0.200 - 2021-05-24

* Use jellyfish-config [Josh Bowling]

## 3.0.199 - 2021-05-24

* Fix lint errors [Josh Bowling]
* Update lint command [Josh Bowling]
* patch: Update dependency @balena/lint to v6 [Renovate Bot]

## 3.0.198 - 2021-05-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.120 [Renovate Bot]

## 3.0.197 - 2021-05-24

* patch: Update internal-patch [Renovate Bot]

## 3.0.196 - 2021-05-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.118 [Renovate Bot]

## 3.0.195 - 2021-05-23

* patch: Update dependency @balena/jellyfish-assert to ^1.1.23 [Renovate Bot]

## 3.0.194 - 2021-05-22

* patch: Update dependency @balena/jellyfish-assert to ^1.1.22 [Renovate Bot]

## 3.0.193 - 2021-05-21

* patch: Update dependency @balena/jellyfish-assert to ^1.1.21 [Renovate Bot]

## 3.0.192 - 2021-05-21

* patch: Update dependency @balena/jellyfish-assert to ^1.1.20 [Renovate Bot]

## 3.0.191 - 2021-05-21

* patch: Update dependency @balena/jellyfish-assert to ^1.1.18 [Renovate Bot]

## 3.0.190 - 2021-05-12

* patch: Update dependency @balena/jellyfish-assert to ^1.1.17 [Renovate Bot]

## 3.0.189 - 2021-05-11

* patch: Update dependency @balena/jellyfish-types to ^0.5.116 [Renovate Bot]

## 3.0.188 - 2021-05-11

* patch: Update non-major [Renovate Bot]

## 3.0.187 - 2021-05-10

* patch: Update dependency @balena/jellyfish-types to ^0.5.115 [Renovate Bot]

## 3.0.186 - 2021-05-08

* build(deps): bump hosted-git-info from 2.8.8 to 2.8.9 [dependabot[bot]]

## 3.0.185 - 2021-05-08

* patch: Update non-major [Renovate Bot]

## 3.0.184 - 2021-05-08

* patch: Update dependency @balena/jellyfish-types to ^0.5.113 [Renovate Bot]

## 3.0.183 - 2021-05-08

* patch: Update dependency @balena/jellyfish-assert to ^1.1.14 [Renovate Bot]

## 3.0.182 - 2021-05-07

* patch: Update dependency lint-staged to v11 [Renovate Bot]

## 3.0.181 - 2021-05-06

* patch: Update non-major [Renovate Bot]

## 3.0.180 - 2021-05-05

* patch: Update dependency catch-uncommitted to v2 [Renovate Bot]

## 3.0.179 - 2021-05-05

* patch: Update dependency @balena/jellyfish-assert to ^1.1.12 [Renovate Bot]

## 3.0.178 - 2021-05-05

* patch: Update non-major [Renovate Bot]

## 3.0.177 - 2021-05-05

* patch: Update dependency @balena/jellyfish-types to ^0.5.110 [Renovate Bot]

## 3.0.176 - 2021-05-05

* patch: Update dependency @balena/jellyfish-types to ^0.5.109 [Renovate Bot]

## 3.0.175 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.108 [Renovate Bot]

## 3.0.174 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.107 [Renovate Bot]

## 3.0.173 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.106 [Renovate Bot]

## 3.0.172 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.105 [Renovate Bot]

## 3.0.171 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.103 [Renovate Bot]

## 3.0.170 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.102 [Renovate Bot]

## 3.0.169 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.101 [Renovate Bot]

## 3.0.168 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.100 [Renovate Bot]

## 3.0.167 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.99 [Renovate Bot]

## 3.0.166 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.98 [Renovate Bot]

## 3.0.165 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.97 [Renovate Bot]

## 3.0.164 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.96 [Renovate Bot]

## 3.0.163 - 2021-05-04

* patch: Update dependency @balena/jellyfish-types to ^0.5.95 [Renovate Bot]

## 3.0.162 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.94 [Renovate Bot]

## 3.0.161 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.92 [Renovate Bot]

## 3.0.160 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.91 [Renovate Bot]

## 3.0.159 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.90 [Renovate Bot]

## 3.0.158 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.89 [Renovate Bot]

## 3.0.157 - 2021-05-03

* patch: Update non-major [Renovate Bot]

## 3.0.156 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.87 [Renovate Bot]

## 3.0.155 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.86 [Renovate Bot]

## 3.0.154 - 2021-05-03

* patch: Update dependency @balena/jellyfish-types to ^0.5.85 [Renovate Bot]

## 3.0.153 - 2021-05-02

* patch: Update dependency @balena/jellyfish-types to ^0.5.84 [Renovate Bot]

## 3.0.152 - 2021-05-02

* patch: Update dependency @balena/jellyfish-types to ^0.5.83 [Renovate Bot]

## 3.0.151 - 2021-05-02

* patch: Update dependency @balena/jellyfish-types to ^0.5.82 [Renovate Bot]

## 3.0.150 - 2021-05-02

* patch: Update dependency @balena/jellyfish-types to ^0.5.81 [Renovate Bot]

## 3.0.149 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.79 [Renovate Bot]

## 3.0.148 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.78 [Renovate Bot]

## 3.0.147 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.77 [Renovate Bot]

## 3.0.146 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.76 [Renovate Bot]

## 3.0.145 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.75 [Renovate Bot]

## 3.0.144 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.74 [Renovate Bot]

## 3.0.143 - 2021-05-01

* patch: Update dependency @balena/jellyfish-types to ^0.5.73 [Renovate Bot]

## 3.0.142 - 2021-04-30

* patch: Update dependency @balena/jellyfish-types to ^0.5.72 [Renovate Bot]

## 3.0.141 - 2021-04-30

* patch: Update dependency @balena/jellyfish-types to ^0.5.71 [Renovate Bot]

## 3.0.140 - 2021-04-30

* patch: Update dependency @balena/jellyfish-types to ^0.5.70 [Renovate Bot]

## 3.0.139 - 2021-04-30

* patch: Update dependency @balena/jellyfish-types to ^0.5.69 [Renovate Bot]

## 3.0.138 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.68 [Renovate Bot]

## 3.0.137 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.67 [Renovate Bot]

## 3.0.136 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.66 [Renovate Bot]

## 3.0.135 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.65 [Renovate Bot]

## 3.0.134 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.64 [Renovate Bot]

## 3.0.133 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.63 [Renovate Bot]

## 3.0.132 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.62 [Renovate Bot]

## 3.0.131 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.61 [Renovate Bot]

## 3.0.130 - 2021-04-29

* patch: Update dependency @balena/jellyfish-types to ^0.5.60 [Renovate Bot]

## 3.0.129 - 2021-04-27

* patch: Update dependency @balena/jellyfish-types to ^0.5.59 [Renovate Bot]

## 3.0.128 - 2021-04-27

* patch: Update dependency @balena/jellyfish-assert to ^1.1.10 [Renovate Bot]

## 3.0.127 - 2021-04-26

* patch: Update dependency @balena/jellyfish-types to ^0.5.58 [Renovate Bot]

## 3.0.126 - 2021-04-26

* patch: Update dependency @balena/jellyfish-types to ^0.5.57 [Renovate Bot]

## 3.0.125 - 2021-04-26

* patch: Update dependency @types/jest to ^26.0.23 [Renovate Bot]

## 3.0.124 - 2021-04-26

* patch: Update dependency @balena/jellyfish-types to ^0.5.55 [Renovate Bot]

## 3.0.123 - 2021-04-26

* patch: Update dependency @balena/jellyfish-types to ^0.5.54 [Renovate Bot]

## 3.0.122 - 2021-04-25

* patch: Update dependency @balena/jellyfish-types to ^0.5.53 [Renovate Bot]

## 3.0.121 - 2021-04-25

* patch: Update dependency @balena/jellyfish-types to ^0.5.52 [Renovate Bot]

## 3.0.120 - 2021-04-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.51 [Renovate Bot]

## 3.0.119 - 2021-04-24

* patch: Update non-major [Renovate Bot]

## 3.0.118 - 2021-04-24

* patch: Update non-major [Renovate Bot]

## 3.0.117 - 2021-04-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.48 [Renovate Bot]

## 3.0.116 - 2021-04-24

* patch: Update dependency @balena/jellyfish-types to ^0.5.47 [Renovate Bot]

## 3.0.115 - 2021-04-23

* patch: Update dependency @balena/jellyfish-types to ^0.5.46 [Renovate Bot]

## 3.0.114 - 2021-04-23

* patch: Update dependency @balena/jellyfish-types to ^0.5.45 [Renovate Bot]

## 3.0.113 - 2021-04-23

* patch: Update non-major [Renovate Bot]

## 3.0.112 - 2021-04-23

* patch: Update dependency typedoc to ^0.20.36 [Renovate Bot]

## 3.0.111 - 2021-04-23

* patch: Update dependency @balena/jellyfish-types to ^0.5.42 [Renovate Bot]

## 3.0.110 - 2021-04-21

* patch: Update dependency @balena/jellyfish-types to ^0.5.41 [Renovate Bot]

## 3.0.109 - 2021-04-21

* patch: Update dependency @balena/jellyfish-types to ^0.5.40 [Renovate Bot]

## 3.0.108 - 2021-04-21

* patch: Update dependency @balena/jellyfish-types to ^0.5.39 [Renovate Bot]

## 3.0.107 - 2021-04-21

* patch: Update dependency @balena/jellyfish-types to ^0.5.38 [Renovate Bot]

## 3.0.106 - 2021-04-20

* patch: Update dependency @balena/jellyfish-types to ^0.5.33 [Renovate Bot]

## 3.0.105 - 2021-04-19

* patch: Update dependency @balena/jellyfish-types to ^0.5.30 [Renovate Bot]

## 3.0.104 - 2021-04-19

* patch: Update dependency @balena/jellyfish-types to ^0.5.29 [Renovate Bot]

## 3.0.103 - 2021-04-18

* patch: Update dependency @balena/jellyfish-types to ^0.5.28 [Renovate Bot]

## 3.0.102 - 2021-04-18

* patch: Update dependency @balena/jellyfish-types to ^0.5.27 [Renovate Bot]

## 3.0.101 - 2021-04-18

* patch: Update dependency @balena/jellyfish-types to ^0.5.26 [Renovate Bot]

## 3.0.100 - 2021-04-18

* patch: Update dependency @balena/jellyfish-types to ^0.5.25 [Renovate Bot]

## 3.0.99 - 2021-04-18

* patch: Update dependency @balena/jellyfish-types to ^0.5.24 [Renovate Bot]

## 3.0.98 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.23 [Renovate Bot]

## 3.0.97 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.22 [Renovate Bot]

## 3.0.96 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.21 [Renovate Bot]

## 3.0.95 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.20 [Renovate Bot]

## 3.0.94 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.19 [Renovate Bot]

## 3.0.93 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.18 [Renovate Bot]

## 3.0.92 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.17 [Renovate Bot]

## 3.0.91 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.16 [Renovate Bot]

## 3.0.90 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.15 [Renovate Bot]

## 3.0.89 - 2021-04-17

* patch: Update dependency @balena/jellyfish-types to ^0.5.14 [Renovate Bot]

## 3.0.88 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.13 [Renovate Bot]

## 3.0.87 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.12 [Renovate Bot]

## 3.0.86 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.11 [Renovate Bot]

## 3.0.85 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.10 [Renovate Bot]

## 3.0.84 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.9 [Renovate Bot]

## 3.0.83 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.8 [Renovate Bot]

## 3.0.82 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.7 [Renovate Bot]

## 3.0.81 - 2021-04-16

* patch: Update dependency @balena/jellyfish-types to ^0.5.6 [Renovate Bot]

## 3.0.80 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.5.5 [Renovate Bot]

## 3.0.79 - 2021-04-15

* patch: Update non-major [Renovate Bot]

## 3.0.78 - 2021-04-15

* patch: Update non-major [Renovate Bot]

## 3.0.77 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.5.2 [Renovate Bot]

## 3.0.76 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.5.1 [Renovate Bot]

## 3.0.75 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.5.0 [Renovate Bot]

## 3.0.74 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.4.74 [Renovate Bot]

## 3.0.73 - 2021-04-15

* patch: Update dependency @balena/jellyfish-types to ^0.4.72 [Renovate Bot]

## 3.0.72 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.70 [Renovate Bot]
* patch: Update dependency @balena/jellyfish-types to ^0.4.68 [Renovate Bot]

## 3.0.71 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.66 [Renovate Bot]

## 3.0.70 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.64 [Renovate Bot]

## 3.0.69 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.63 [Renovate Bot]

## 3.0.68 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.62 [Renovate Bot]

## 3.0.67 - 2021-04-14

* patch: Update dependency @balena/jellyfish-types to ^0.4.61 [Renovate Bot]

## 3.0.66 - 2021-04-13

* patch: Update dependency @balena/jellyfish-types to ^0.4.60 [Renovate Bot]

## 3.0.65 - 2021-04-13

* patch: Update dependency @balena/jellyfish-types to ^0.4.59 [Renovate Bot]

## 3.0.64 - 2021-04-13

* patch: Update dependency @balena/jellyfish-types to ^0.4.58 [Renovate Bot]

## 3.0.63 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.57 [Renovate Bot]

## 3.0.62 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.56 [Renovate Bot]

## 3.0.61 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.55 [Renovate Bot]

## 3.0.60 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.54 [Renovate Bot]

## 3.0.59 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.53 [Renovate Bot]

## 3.0.58 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.52 [Renovate Bot]

## 3.0.57 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.51 [Renovate Bot]

## 3.0.56 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.50 [Renovate Bot]

## 3.0.55 - 2021-04-12

* patch: Update dependency @balena/jellyfish-types to ^0.4.48 [Renovate Bot]

## 3.0.54 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.47 [Renovate Bot]

## 3.0.53 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.46 [Renovate Bot]

## 3.0.52 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.45 [Renovate Bot]

## 3.0.51 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.44 [Renovate Bot]

## 3.0.50 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.43 [Renovate Bot]

## 3.0.49 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.42 [Renovate Bot]

## 3.0.48 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.41 [Renovate Bot]

## 3.0.47 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.40 [Renovate Bot]

## 3.0.46 - 2021-04-11

* patch: Update dependency @balena/jellyfish-types to ^0.4.39 [Renovate Bot]

## 3.0.45 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.38 [Renovate Bot]

## 3.0.44 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.35 [Renovate Bot]

## 3.0.43 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.33 [Renovate Bot]

## 3.0.42 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.32 [Renovate Bot]

## 3.0.41 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.31 [Renovate Bot]

## 3.0.40 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.30 [Renovate Bot]

## 3.0.39 - 2021-04-10

* patch: Update dependency @balena/jellyfish-types to ^0.4.29 [Renovate Bot]

## 3.0.38 - 2021-04-07

* patch: Update non-major [Renovate Bot]

## 3.0.37 - 2021-04-07

* patch: Update dependency @balena/jellyfish-types to ^0.4.27 [Renovate Bot]

## 3.0.36 - 2021-04-06

* patch: Update dependency @balena/jellyfish-types to ^0.4.26 [Renovate Bot]

## 3.0.35 - 2021-04-06

* patch: Update dependency @balena/jellyfish-types to ^0.4.25 [Renovate Bot]

## 3.0.34 - 2021-04-06

* patch: Update dependency @balena/jellyfish-types to ^0.4.24 [Renovate Bot]

## 3.0.33 - 2021-04-06

* patch: Update dependency @balena/jellyfish-types to ^0.4.23 [Renovate Bot]

## 3.0.32 - 2021-04-06

* patch: Update dependency @balena/jellyfish-types to ^0.4.21 [Renovate Bot]

## 3.0.31 - 2021-04-04

* patch: Update dependency @balena/jellyfish-types to ^0.4.20 [Renovate Bot]

## 3.0.30 - 2021-04-04

* patch: Update dependency @balena/jellyfish-types to ^0.4.19 [Renovate Bot]

## 3.0.29 - 2021-04-04

* patch: Update dependency @balena/jellyfish-types to ^0.4.18 [Renovate Bot]

## 3.0.28 - 2021-04-04

* patch: Update dependency @balena/jellyfish-types to ^0.4.17 [Renovate Bot]

## 3.0.27 - 2021-04-04

* patch: Update non-major [Renovate Bot]

## 3.0.26 - 2021-04-03

* patch: Update dependency @types/object-hash to v2 [Renovate Bot]

## 3.0.25 - 2021-04-03

* patch: Update dependency @balena/jellyfish-types to ^0.4.14 [Renovate Bot]

## 3.0.24 - 2021-04-02

* patch: Update dependency @balena/jellyfish-types to ^0.4.12 [Renovate Bot]

## 3.0.23 - 2021-04-02

* patch: Update dependency @balena/jellyfish-assert to ^1.1.4 [Renovate Bot]

## 3.0.22 - 2021-04-02

* patch: Update non-major [Renovate Bot]

## 3.0.21 - 2021-04-02

* patch: Update dependency @balena/jellyfish-types to ^0.4.10 [Renovate Bot]

## 3.0.20 - 2021-04-02

* patch: Update non-major [Renovate Bot]

## 3.0.19 - 2021-04-02

* patch: Update dependency @balena/jellyfish-types to ^0.4.8 [Renovate Bot]

## 3.0.18 - 2021-04-01

* patch: Update dependency @balena/jellyfish-types to ^0.4.7 [Renovate Bot]

## 3.0.17 - 2021-04-01

* patch: Update dependency @balena/jellyfish-types to ^0.4.6 [Renovate Bot]

## 3.0.16 - 2021-03-31

* patch: Update dependency @balena/jellyfish-assert to ^1.1.3 [Renovate Bot]

## 3.0.15 - 2021-03-31

* patch: Update dependency @balena/jellyfish-assert to ^1.1.2 [Renovate Bot]

## 3.0.14 - 2021-03-30

* patch: Update dependency @formulajs/formulajs to ^2.6.8 [Renovate Bot]

## 3.0.13 - 2021-03-29

* patch: Update dependency @balena/jellyfish-types to ^0.4.4 [Renovate Bot]

## 3.0.12 - 2021-03-28

* patch: Update dependency @formulajs/formulajs to ^2.6.7 [Renovate Bot]

## 3.0.11 - 2021-03-26

* patch: Update dependency @balena/jellyfish-types to ^0.4.0 [Renovate Bot]

## 3.0.10 - 2021-03-26

* Replace husky with simple-git-hooks [Graham McCulloch]

## 3.0.9 - 2021-03-26

* patch: Update dependency @balena/jellyfish-types to ^0.2.2 [Renovate Bot]

## 3.0.8 - 2021-03-26

* patch: Update dependency @balena/jellyfish-types to ^0.2.0 [Renovate Bot]

## 3.0.7 - 2021-03-25

* patch: Update dependency @balena/jellyfish-assert to ^1.1.1 [Renovate Bot]

## 3.0.6 - 2021-03-25

* patch: Update dependency @types/jest to ^26.0.22 [Renovate Bot]

## 3.0.5 - 2021-03-25

* patch: Update dependency @balena/jellyfish-assert to ^1.1.0 [Renovate Bot]

## 3.0.4 - 2021-03-25

* patch: Update non-major [Renovate Bot]

## 3.0.3 - 2021-03-25

* Publish Typedoc docs using a GitHub action [Graham McCulloch]

## 3.0.2 - 2021-03-25

* patch: Update dependency @balena/jellyfish-assert to ^1.0.68 [Renovate Bot]

## 3.0.1 - 2021-03-24

* patch: Update dependency @formulajs/formulajs to ^2.6.6 [Renovate Bot]

## 3.0.0 - 2021-03-24

* Update docs [Graham McCulloch]
* Converted to TypeScript [Graham McCulloch]

## 2.1.62 - 2021-03-22

* patch: Update dependency @balena/jellyfish-assert to ^1.0.67 [Renovate Bot]

## 2.1.61 - 2021-03-22

* patch: Update dependency @balena/jellyfish-assert to ^1.0.66 [Renovate Bot]

## 2.1.60 - 2021-03-21

* patch: Update non-major [Renovate Bot]

## 2.1.59 - 2021-03-18

* patch: Update dependency @balena/jellyfish-assert to ^1.0.64 [Renovate Bot]

## 2.1.58 - 2021-03-15

* patch: Update dependency eslint-plugin-jsdoc to ^32.3.0 [Renovate Bot]

## 2.1.57 - 2021-03-13

* patch: Update dependency eslint to ^7.22.0 [Renovate Bot]

## 2.1.56 - 2021-03-12

* patch: Update dependency @balena/jellyfish-assert to ^1.0.63 [Renovate Bot]

## 2.1.55 - 2021-03-10

* patch: Update dependency eslint-plugin-lodash to ^7.2.0 [Renovate Bot]

## 2.1.54 - 2021-03-04

* Don't evaluate AGGREGATE formulas for create and update contracts [Lucian Buzzo]

## 2.1.53 - 2021-03-02

* patch: Update non-major [Renovate Bot]

## 2.1.52 - 2021-03-01

* patch: Update non-major [Renovate Bot]

## 2.1.51 - 2021-02-27

* patch: Update dependency jsdoc-to-markdown to v7 [Renovate Bot]

## 2.1.50 - 2021-02-27

* patch: Update dependency eslint to ^7.21.0 [Renovate Bot]

## 2.1.49 - 2021-02-27

* patch: Update dependency @formulajs/formulajs to ^2.6.5 [Renovate Bot]

## 2.1.48 - 2021-02-26

* patch: Update dependency @formulajs/formulajs to ^2.6.4 [Renovate Bot]

## 2.1.47 - 2021-02-26

* patch: Update dependency @balena/jellyfish-assert to ^1.0.60 [Renovate Bot]

## 2.1.46 - 2021-02-23

* patch: Update dependency eslint-plugin-jsdoc to ^32.2.0 [Renovate Bot]

## 2.1.45 - 2021-02-23

* patch: Update dependency eslint-plugin-jsdoc to ^32.1.1 [Renovate Bot]

## 2.1.44 - 2021-02-22

* patch: Update dependency husky to ^5.1.1 [Renovate Bot]

## 2.1.43 - 2021-02-21

* patch: Update dependency husky to ^5.1.0 [Renovate Bot]

## 2.1.42 - 2021-02-21

* patch: Update dependency eslint-plugin-jsdoc to ^32.1.0 [Renovate Bot]

## 2.1.41 - 2021-02-20

* patch: Update dependency lodash to ^4.17.21 [Renovate Bot]

## 2.1.40 - 2021-02-20

* patch: Update dependency @balena/jellyfish-assert to ^1.0.59 [Renovate Bot]

## 2.1.39 - 2021-02-20

* Add versionbot CHANGELOG.yml [Josh Bowling]

# v2.1.38
## (2021-02-17)

* patch: Update dependency eslint-plugin-jsdoc to ^32.0.2 [Renovate Bot]

# v2.1.37
## (2021-02-16)

* patch: Update dependency depcheck to ^1.4.0 [Renovate Bot]

# v2.1.36
## (2021-02-16)

* patch: Update dependency eslint-plugin-jsdoc to ^32.0.1 [Renovate Bot]

# v2.1.35
## (2021-02-15)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.58 [Renovate Bot]

# v2.1.34
## (2021-02-15)

* patch: Update dependency eslint-plugin-jsdoc to v32 [Renovate Bot]

# v2.1.33
## (2021-02-13)

* patch: Update dependency eslint to ^7.20.0 [Renovate Bot]

# v2.1.32
## (2021-02-11)

* patch: Update dependency @formulajs/formulajs to ^2.6.3 [Renovate Bot]

# v2.1.31
## (2021-02-11)

* patch: Update dependency eslint-plugin-promise to ^4.3.1 [Renovate Bot]

# v2.1.30
## (2021-02-09)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.57 [Renovate Bot]

# v2.1.29
## (2021-02-08)

* patch: Update dependency husky to ^5.0.9 [Renovate Bot]

# v2.1.28
## (2021-02-07)

* patch: Update dependency eslint-plugin-jsdoc to ^31.6.1 [Renovate Bot]

# v2.1.27
## (2021-02-06)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.56 [Renovate Bot]

# v2.1.26
## (2021-02-06)

* Use shared renovate config base [Josh Bowling]

# v2.1.25
## (2021-02-05)

* patch: Update dependency lint-staged to ^10.5.4 [Renovate Bot]

# v2.1.24
## (2021-02-05)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.55 [Renovate Bot]

# v2.1.23
## (2021-02-05)

* Set renovate npm version [Josh Bowling]

# v2.1.22
## (2021-02-03)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.54 [Renovate Bot]

# v2.1.21
## (2021-02-02)

* Add .whitesource configuration file [whitesource-bolt-for-github[bot]]

# v2.1.20
## (2021-02-02)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.53 [Renovate Bot]

# v2.1.19
## (2021-02-01)

* patch: Update dependency deplint to ^1.1.3 [Renovate Bot]

# v2.1.18
## (2021-01-31)

* patch: Update dependency eslint-plugin-jsdoc to ^31.6.0 [Renovate Bot]

# v2.1.17
## (2021-01-31)

* patch: Update dependency eslint-plugin-jsdoc to ^31.5.0 [Renovate Bot]

# v2.1.16
## (2021-01-31)

* patch: Update dependency eslint to ^7.19.0 [Renovate Bot]

# v2.1.15
## (2021-01-29)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.52 [Renovate Bot]

# v2.1.14
## (2021-01-28)

* patch: Update dependency husky to ^5.0.8 [Renovate Bot]

# v2.1.13
## (2021-01-27)

* patch: Update dependency eslint-plugin-jsdoc to ^31.4.0 [Renovate Bot]

# v2.1.12
## (2021-01-27)

* patch: Update dependency husky to ^5.0.7 [Renovate Bot]

# v2.1.11
## (2021-01-25)

* patch: Update dependency eslint-plugin-jsdoc to ^31.3.3 [Renovate Bot]

# v2.1.10
## (2021-01-24)

* patch: Update dependency eslint-plugin-jsdoc to ^31.3.2 [Renovate Bot]

# v2.1.9
## (2021-01-24)

* patch: Update dependency eslint-plugin-jsdoc to ^31.3.1 [Renovate Bot]

# v2.1.8
## (2021-01-23)

* patch: Update dependency eslint-plugin-jsdoc to ^31.2.2 [Renovate Bot]

# v2.1.7
## (2021-01-23)

* patch: Update dependency eslint-plugin-jsdoc to ^31.2.1 [Renovate Bot]

# v2.1.6
## (2021-01-23)

* patch: Update dependency eslint-plugin-jsdoc to ^31.2.0 [Renovate Bot]

# v2.1.5
## (2021-01-23)

* patch: Update dependency eslint-plugin-jsdoc to ^31.1.0 [Renovate Bot]

# v2.1.4
## (2021-01-20)

* patch: Update dependency eslint-plugin-jsdoc to ^31.0.8 [Renovate Bot]

# v2.1.3
## (2021-01-18)

* patch: Update non-major [Renovate Bot]

# v2.1.2
## (2021-01-17)

* patch: Update dependency eslint-plugin-jsdoc to ^31.0.6 [Renovate Bot]

# v2.1.1
## (2021-01-16)

* patch: Update non-major [Renovate Bot]

# v2.1.0
## (2021-01-15)

* Add support for UNIQUE(FLATMAP()) as an alternative to AGGREGATE [Graham McCulloch]

# v2.0.43
## (2021-01-15)

* patch: Update dependency eslint-plugin-jsdoc to ^31.0.5 [Renovate Bot]

# v2.0.42
## (2021-01-14)

* patch: Update dependency eslint-plugin-jsdoc to ^31.0.4 [Renovate Bot]

# v2.0.41
## (2021-01-14)

* Execute git add in lint-staged [Josh Bowling]

# v2.0.40
## (2021-01-11)

* patch: Update dependency eslint-plugin-jsdoc to v31 [Renovate Bot]

# v2.0.39
## (2021-01-11)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.49 [Renovate Bot]

# v2.0.38
## (2021-01-11)

* Use company token for renovate [Josh Bowling]

# v2.0.37
## (2021-01-11)

* patch: Update dependency @balena/jellycheck to ^0.1.1 [Renovate Bot]

# v2.0.36
## (2021-01-09)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.48 [Renovate Bot]

# v2.0.35
## (2021-01-08)

* patch: Update dependency @formulajs/formulajs to ^2.6.2 [Renovate Bot]

# v2.0.34
## (2021-01-07)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.47 [Renovate Bot]

# v2.0.33
## (2021-01-07)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.46 [Renovate Bot]

# v2.0.32
## (2021-01-07)

* patch: Update non-major [Renovate Bot]

# v2.0.31
## (2021-01-01)

* patch: Update dependency ava to ^3.15.0 [Renovate Bot]

# v2.0.30
## (2020-12-30)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.44 [Renovate Bot]

# v2.0.29
## (2020-12-30)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.43 [Renovate Bot]

# v2.0.28
## (2020-12-30)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.42 [Renovate Bot]

# v2.0.27
## (2020-12-27)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.41 [Renovate Bot]

# v2.0.26
## (2020-12-25)

* patch: Update dependency object-hash to ^2.1.1 [Renovate Bot]

# v2.0.25
## (2020-12-23)

* patch: Update dependency @balena/jellycheck to ^0.1.0 [Renovate Bot]

# v2.0.24
## (2020-12-19)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.9 [Renovate Bot]

# v2.0.23
## (2020-12-19)

* patch: Update dependency eslint to ^7.16.0 [Renovate Bot]

# v2.0.22
## (2020-12-15)

* patch: Update dependency @balena/jellycheck to ^0.0.9 [Renovate Bot]

# v2.0.21
## (2020-12-14)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.40 [Renovate Bot]

# v2.0.20
## (2020-12-12)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.39 [Renovate Bot]

# v2.0.19
## (2020-12-12)

* build(deps): bump ini from 1.3.5 to 1.3.8 [dependabot[bot]]

# v2.0.18
## (2020-12-12)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.38 [Renovate Bot]

# v2.0.17
## (2020-12-12)

* Make postinstall cross shell compatible [Josh Bowling]

# v2.0.16
## (2020-12-12)

* patch: Update non-major [Renovate Bot]

# v2.0.15
## (2020-12-12)

* renovate: Ignore node version bumps [Josh Bowling]

# v2.0.14
## (2020-12-10)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.36 [Renovate Bot]

# v2.0.13
## (2020-12-10)

* Set as root in eslint config file [Graham McCulloch]

# v2.0.12
## (2020-12-08)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.35 [Renovate Bot]

# v2.0.11
## (2020-12-08)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.34 [Renovate Bot]

# v2.0.10
## (2020-12-08)

* Add husky postinstall [Josh Bowling]

# v2.0.9
## (2020-12-07)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.33 [Renovate Bot]

# v2.0.8
## (2020-12-07)

* patch: Update dependency ava to ^3.14.0 [Renovate Bot]
* Remove husky postinstall [Josh Bowling]
* Migrate husky configuration for v5 [Josh Bowling]
* patch: Update dependency husky to v5 [Renovate Bot]

# v2.0.7
## (2020-12-05)

* patch: Update dependency husky to ^4.3.4 [Renovate Bot]

# v2.0.6
## (2020-12-05)

* patch: Update dependency eslint to ^7.15.0 [Renovate Bot]

# v2.0.5
## (2020-12-04)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.32 [Renovate Bot]

# v2.0.4
## (2020-12-04)

* patch: Update dependency lint-staged to ^10.5.3 [Renovate Bot]

# v2.0.3
## (2020-12-01)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.31 [Renovate Bot]

# v2.0.2
## (2020-11-24)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.30 [Renovate Bot]

# v2.0.1
## (2020-11-24)

* patch: Update dependency lint-staged to ^10.5.2 [Renovate Bot]

# v2.0.0
## (2020-11-23)

* Use schedule instead of async in triggered actions [Graham McCulloch]

# v1.0.42
## (2020-11-20)

* patch: Update dependency eslint to ^7.14.0 [Renovate Bot]

# v1.0.41
## (2020-11-20)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.29 [Renovate Bot]

# v1.0.40
## (2020-11-19)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.28 [Renovate Bot]

# v1.0.39
## (2020-11-19)

* patch: Update dependency eslint-config-standard to ^16.0.2 [Renovate Bot]

# v1.0.38
## (2020-11-14)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.8 [Renovate Bot]

# v1.0.37
## (2020-11-12)

* Require Node v14 [Josh Bowling]

# v1.0.36
## (2020-11-11)

* Use Node v14 during CI tests [Josh Bowling]

# v1.0.35
## (2020-11-10)

* patch: Update dependency depcheck to ^1.3.1 [Renovate Bot]

# v1.0.34
## (2020-11-09)

* patch: Update dependency depcheck to ^1.3.0 [Renovate Bot]

# v1.0.33
## (2020-11-07)

* patch: Update dependency eslint to ^7.13.0 [Renovate Bot]

# v1.0.32
## (2020-11-06)

* patch: Update dependency @formulajs/formulajs to ^2.6.1 [Renovate Bot]

# v1.0.31
## (2020-11-06)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.23 [Renovate Bot]

# v1.0.30
## (2020-11-06)

* patch: Update dependency @balena/jellycheck to ^0.0.8 [Renovate Bot]

# v1.0.29
## (2020-11-06)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.7 [Renovate Bot]

# v1.0.28
## (2020-11-06)

* Force Renovate to use node 14 and npm 6 [Josh Bowling]

# v1.0.27
## (2020-11-04)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.22 [Renovate Bot]

# v1.0.26
## (2020-11-03)

* patch: Update dependency @formulajs/formulajs to ^2.6.0 [Renovate Bot]

# v1.0.25
## (2020-11-02)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.21 [Renovate Bot]

# v1.0.24
## (2020-11-02)

* Remove eslint-plugin-standard [Josh Bowling]
* patch: Update dependency eslint-config-standard to v16 [Renovate Bot]

# v1.0.23
## (2020-11-01)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.6 [Renovate Bot]

# v1.0.22
## (2020-10-31)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.20 [Renovate Bot]

# v1.0.21
## (2020-10-31)

* patch: Update dependency lint-staged to ^10.5.1 [Renovate Bot]

# v1.0.20
## (2020-10-31)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.5 [Renovate Bot]

# v1.0.19
## (2020-10-31)

* patch: Update dependency eslint-plugin-jsdoc to ^30.7.4 [Renovate Bot]

# v1.0.18
## (2020-10-30)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.19 [Renovate Bot]

# v1.0.17
## (2020-10-28)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.18 [Renovate Bot]

# v1.0.16
## (2020-10-28)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.17 [Renovate Bot]

# v1.0.15
## (2020-10-28)

* Renovate config set pruneStaleBranches to false [Marios Balamatsias]
* patch: Update dependency @balena/jellyfish-assert to ^1.0.16 [Renovate Bot]
* patch: Update dependency eslint-config-standard to ^15.0.1 [Renovate Bot]
* patch: Update dependency eslint to ^7.12.1 [Renovate Bot]
* patch: Update dependency lint-staged to ^10.5.0 [Renovate Bot]
* patch: Update dependency eslint to ^7.12.0 [Renovate Bot]
* patch: Update dependency eslint-config-standard to v15 [Renovate Bot]
* patch: Update dependency eslint-plugin-standard to ^4.0.2 [Renovate Bot]
* patch: Update dependency @formulajs/formulajs to ^2.5.0 [Renovate Bot]
* patch: Update dependency eslint-plugin-jsdoc to ^30.7.3 [Renovate Bot]
* patch: Update dependency eslint-plugin-jsdoc to ^30.7.2 [Renovate Bot]
* patch: Update dependency eslint-plugin-jsdoc to ^30.7.1 [Renovate Bot]
* patch: Update dependency lint-staged to ^10.4.2 [Renovate Bot]
* patch: Update dependency lint-staged to ^10.4.1 [Renovate Bot]

# v1.0.14
## (2020-10-14)

* patch: Update dependency eslint-plugin-jsdoc to ^30.6.5 [Renovate Bot]

# v1.0.13
## (2020-10-09)

* patch: Update dependency eslint to ^7.11.0 [Renovate Bot]

# v1.0.12
## (2020-10-09)

* patch: Update dependency eslint-plugin-jsdoc to ^30.6.4 [Renovate Bot]

# v1.0.11
## (2020-10-02)

* patch: Update dependency eslint-plugin-jsdoc to ^30.6.3 [Renovate Bot]

# v1.0.10
## (2020-09-29)

* patch: Update dependency eslint-plugin-jsdoc to ^30.6.2 [Renovate Bot]

# v1.0.9
## (2020-09-28)

* patch: Update dependency eslint-plugin-jsdoc to ^30.6.1 [Renovate Bot]

# v1.0.8
## (2020-09-28)

* patch: Update non-major [Renovate Bot]

# v1.0.7
## (2020-09-26)

* patch: Update dependency eslint to ^7.10.0 [Renovate Bot]

# v1.0.6
## (2020-09-25)

* patch: Update dependency eslint-plugin-jsdoc to ^30.5.2 [Renovate Bot]

# v1.0.5
## (2020-09-22)

* Revert "Extract formula into own file. Add DATE_VALUE to formulas. Replace FLIP" [Lucy-Jane]

# v1.0.4
## (2020-09-20)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.15 [Renovate Bot]

# v1.0.3
## (2020-09-18)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.14 [Renovate Bot]

# v1.0.2
## (2020-09-16)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.13 [Renovate Bot]

# v1.0.1
## (2020-09-16)

* patch: Update non-major [Renovate Bot]

# v1.0.0
## (2020-09-15)

* Extract formula into own file. Add DATE_VALUE to formulas. Replace FLIP and PARTIAL with PARTIAL_RIGHT. Rename PROPERTY formula to GET_PROPERTY to make it clearer and easier to understand. Move tests into seperate files and add tests for DATE_VALUE. Move checks for special cases in the getTypeTrigger method to its own method called checkForSpecialCases and expand it to include checks for the DATE_VALUE formula [Lucy-Jane]

# v0.0.56
## (2020-09-14)

* patch: Update dependency eslint-plugin-jsdoc to ^30.5.1 [Renovate Bot]

# v0.0.55
## (2020-09-12)

* patch: Update dependency eslint to ^7.9.0 [Renovate Bot]

# v0.0.54
## (2020-09-12)

* patch: Update dependency eslint-plugin-jsdoc to ^30.4.2 [Renovate Bot]

# v0.0.53
## (2020-09-11)

* patch: Update dependency eslint-plugin-jsdoc to ^30.4.1 [Renovate Bot]

# v0.0.52
## (2020-09-10)

* Use engine-strict flag to check node version [Josh Bowling]

# v0.0.51
## (2020-09-09)

* patch: Update non-major [Renovate Bot]

# v0.0.50
## (2020-09-06)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.10 [Renovate Bot]

# v0.0.49
## (2020-09-06)

* ci: Remove node 10 tests from balenaCI config [Josh Bowling]

# v0.0.48
## (2020-09-06)

* ci: Add default FILES glob [Josh Bowling]

# v0.0.47
## (2020-09-06)

* patch: Update dependency eslint-plugin-jsdoc to ^30.3.2 [Renovate Bot]

# v0.0.46
## (2020-09-05)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.9 [Renovate Bot]

# v0.0.45
## (2020-09-03)

* patch: Update non-major [Renovate Bot]

# v0.0.44
## (2020-09-03)

* patch: Update non-major [Renovate Bot]

# v0.0.43
## (2020-09-01)

* patch: Update dependency eslint to ^7.8.1 [Renovate Bot]

# v0.0.42
## (2020-09-01)

* patch: Update dependency @balena/jellycheck to ^0.0.7 [Renovate Bot]

# v0.0.41
## (2020-09-01)

* ci: Replace scripts/ with jellycheck [Josh Bowling]

# v0.0.40
## (2020-09-01)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.6 [Renovate Bot]

# v0.0.39
## (2020-09-01)

* ci: Add lint-staged precommit [Josh Bowling]

# v0.0.38
## (2020-08-31)

* patch: Update dependency eslint to ^7.8.0 [Renovate Bot]

# v0.0.37
## (2020-08-31)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.5 [Renovate Bot]

# v0.0.36
## (2020-08-31)

* ci: Set commitBody in renovate config [Josh Bowling]

# v0.0.35
## (2020-08-31)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.4 [Renovate Bot]

# v0.0.34
## (2020-08-29)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.3 [Renovate Bot]

# v0.0.33
## (2020-08-26)

* patch: Update dependency @balena/jellyfish-assert to ^1.0.2 [Renovate Bot]

# v0.0.32
## (2020-08-26)

* patch: Update dependency @balena/jellyfish-assert to v1 [Renovate Bot]

# v0.0.31
## (2020-08-26)

* patch: Update dependency eslint-plugin-jsdoc to ^30.3.0 [Renovate Bot]

# v0.0.30
## (2020-08-24)

* patch: Update dependency ava to ^3.12.1 [Renovate Bot]

# v0.0.29
## (2020-08-23)

* patch: Update dependency @balena/jellyfish-assert to ^0.0.28 [Renovate Bot]

# v0.0.28
## (2020-08-23)

* patch: Update dependency ava to ^3.12.0 [Renovate Bot]

# v0.0.27
## (2020-08-18)

* patch: Update dependency @balena/jellyfish-assert to ^0.0.27 [Renovate Bot]

# v0.0.26
## (2020-08-18)

* ci: Remove dev-major group from Renovate config [Josh Bowling]

# v0.0.25
## (2020-08-17)

* patch: Update non-major [Renovate Bot]

# v0.0.24
## (2020-08-16)

* patch: Update dependency @balena/jellyfish-assert to ^0.0.25 [Renovate Bot]

# v0.0.23
## (2020-08-16)

* patch: Update dependency eslint-plugin-jsdoc to ^30.2.3 [Renovate Bot]

# v0.0.22
## (2020-08-15)

* patch: Update dependency @balena/jellyfish-assert to ^0.0.24 [Renovate Bot]

# v0.0.21
## (2020-08-14)

* patch: Update dependency eslint to ^7.7.0 [Renovate Bot]

# v0.0.20
## (2020-08-14)

* patch: Update dependency @balena/jellyfish-assert to ^0.0.23 [Renovate Bot]

# v0.0.19
## (2020-08-13)

* patch: Update non-major [Renovate Bot]

# v0.0.18
## (2020-08-12)

* patch: Update dependency depcheck to ^1.2.0 [Renovate Bot]
* patch: Update dependency @balena/jellyfish-assert to ^0.0.21 [Renovate Bot]

# v0.0.17
## (2020-08-05)

* Add missing space to Makefile [Josh Bowling]

# v0.0.16
## (2020-08-05)

* Add more thorough linting [Josh Bowling]
* patch: Update dependency eslint-plugin-jsdoc to ^30.2.1 [Renovate Bot]
* patch: Update dependency eslint-plugin-jsdoc to ^30.2.0 [Renovate Bot]
* patch: Update dependency ava to ^3.11.1 [Renovate Bot]
* patch: Update non-major [Renovate Bot]

# v0.0.15
## (2020-07-30)

* Caret pinned dependencies [Josh Bowling]
* patch: Update dependency @balena/jellyfish-assert to v0.0.20 [Renovate Bot]

# v0.0.14
## (2020-07-28)

* Add a colon to renovate patch commit prefix [StefKors]
* patch Update dependency ava to ^3.11.0 [Renovate Bot]
* patch Update dependency @balena/jellyfish-assert to v0.0.19 [Renovate Bot]

# v0.0.13
## (2020-07-22)

* ci: Add balenaCI config removing node8 test [Josh Bowling]
* ci: Add label setting to renovate config [Josh Bowling]

# v0.0.12
## (2020-07-21)

* Replace formulajs with @formulajs/formulajs [Josh Bowling]
* patch Update dependency @balena/jellyfish-assert to v0.0.18 [Renovate Bot]
* patch Update dependency eslint-plugin-jsdoc to v30 [Renovate Bot]
* patch Update non-major [Renovate Bot]

# v0.0.11
## (2020-07-21)

* Switch from dependabot to renovate [Josh Bowling]

# v0.0.10
## (2020-07-16)

* Switch to Dependabot Preview [Josh Bowling]

# v0.0.9
## (2020-07-14)

* patch: bump lodash from 4.17.15 to 4.17.19 [dependabot[bot]]

# v0.0.8
## (2020-07-14)

* patch: bump eslint-plugin-jsdoc from 28.6.0 to 29.2.0 [dependabot[bot]]

# v0.0.7
## (2020-07-14)

* patch: bump eslint from 7.3.1 to 7.4.0 [dependabot[bot]]

# v0.0.6
## (2020-07-14)

* patch: bump ava from 3.9.0 to 3.10.1 [dependabot[bot]]

# v0.0.5
## (2020-07-14)

* Add dependabot config [Josh Bowling]

# v0.0.4
## (2020-07-02)

* Update code to include async: true [Josh Bowling]

# v0.0.3
## (2020-07-02)

* docs: Improve description in README [Josh Bowling]

# v0.0.2
## (2020-07-02)

* Copy over library code [Josh Bowling]
